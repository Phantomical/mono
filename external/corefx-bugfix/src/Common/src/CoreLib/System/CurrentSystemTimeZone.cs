...
namespace System
{
    ...
    internal partial class CurrentSystemTimeZone : TimeZone
    {
        ...
        public override TimeSpan GetUtcOffset(DateTime time)
        {
            ...
            {
                // m_ticksOffset is a single base offset and GetTimeZoneData ()
                // has nothing to say outside 1970..2037, so this class cannot
                // express an offset that only ever applied before standard
                // time existed. TimeZoneInfo carries the whole history, and
                // TimeZone.ToLocalTime () already answers from it - going
                // through the same place is what makes the round trip hold.
