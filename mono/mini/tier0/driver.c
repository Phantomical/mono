...
// No arch stub reloads a virtual delegate's receiver and jumps through its
// vtable, so nothing sets MonoDelegateClassMethodPair::is_virtual and neither
// caller reaches this.
gpointer
...
// None of the opcodes below has a case in the arch codegen, so
// mono_decompose_opcode () asks for each of them to be emulated. The table
// this registers into is their only path to code.
void
mono_tier0_register_opcode_emulations (void)
{
...
#endif
	mini_register_opcode_emulation (OP_FCONV_TO_U8, &mono_get_jit_icall_info ()->__emul_fconv_to_u8,
		"__emul_fconv_to_u8", mono_icall_sig_ulong_double, (gpointer)mono_fconv_u8_2, "mono_fconv_u8_2", FALSE);
	mini_register_opcode_emulation (OP_RCONV_TO_U8, &mono_get_jit_icall_info ()->__emul_rconv_to_u8,
		"__emul_rconv_to_u8", mono_icall_sig_ulong_float, (gpointer)mono_rconv_u8, "mono_rconv_u8", FALSE);
#ifdef MONO_ARCH_EMULATE_CONV_R8_UN
	mini_register_opcode_emulation (OP_ICONV_TO_R_UN, &mono_get_jit_icall_info ()->__emul_iconv_to_r_un,
		"__emul_iconv_to_r_un", mono_icall_sig_double_int32, (gpointer)mono_conv_to_r8_un, "mono_conv_to_r8_un", FALSE);
#endif
	mini_register_opcode_emulation (OP_FREM, &mono_get_jit_icall_info ()->__emul_frem,
		"__emul_frem", mono_icall_sig_double_double_double, (gpointer)mono_fmod, "mono_fmod", FALSE);
	mini_register_opcode_emulation (OP_RREM, &mono_get_jit_icall_info ()->__emul_rrem,
		"__emul_rrem", mono_icall_sig_float_float_float, (gpointer)fmodf, "fmodf", FALSE);
}
...
//#define DEBUGSSA_CLASS "Tests"
	...
	if (cfg->verbose_level >= 2) {
		...
		GString *disassembly = g_string_new (NULL);

		mono_disassemble_code (cfg, cfg->native_code, cfg->code_len, id + 3, disassembly);
		g_print ("\n*** ASM for %s ***\n%s***\n\n", id, disassembly->str);
		g_string_free (disassembly, TRUE);
		g_free (id);
	}

	if (mono_tier0_asm_dump_wanted (cfg->method)) {
		char *id = mono_method_full_name (cfg->method, TRUE);
		GString *disassembly = g_string_new (NULL);

		mono_disassemble_code (cfg, cfg->native_code, cfg->code_len, id + 3, disassembly);
		mono_tier0_asm_dump_write (cfg->method, disassembly->str);
		g_string_free (disassembly, TRUE);
		g_free (id);
	}
                    ...
                    MonoError *error)
{
	...
	*out_needs_context = FALSE;
	...
	/*
	 * A generic method definition has no instantiation to compile against.
	 * mono_method_is_generic_impl () does not count an uninflated method as
	 * generic unless its class is itself a generic type definition. So
	 * mini_method_compile () never shares one, and mono_method_to_ir ()
	 * gets the definition itself and asserts. mono_ldftn () compiles
	 * whatever the IL names, and Reflection.Emit writes an ldftn naming a
	 * generic method definition. The backend translates one, so the
	 * refusal sends the method to tier 1. A method of a generic type
	 * definition carries no instantiation either, and is refused alongside
	 * it.
	 */
