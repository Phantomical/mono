...
mono_save_seq_point_info (MonoCompile *cfg, MonoJitInfo *jinfo)
{
	...
	if (!cfg->compile_aot) {
		...
		published = (GSList *)g_hash_table_lookup (domain_jit_info (domain)->seq_points, cfg->method_to_register);
		if (published)
			/* MonoJitDomainInfo::seq_points says why this appends in place. */
			g_slist_append (published, cfg->seq_point_info);
		else
			g_hash_table_insert (domain_jit_info (domain)->seq_points, cfg->method_to_register,
			                     g_slist_append (NULL, cfg->seq_point_info));
