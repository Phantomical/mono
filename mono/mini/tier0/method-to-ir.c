 ...
 * At this point the IR can be handed over to a dumb code generator or to the
 * optimizing code generator that will translate it to SSA form.
 *
...
/// Maps a conv.ovf.i*.un opcode to its counterpart without .un, and leaves any
/// other opcode alone.
static int
conv_ovf_dropping_un (int il_op)
{
	switch (il_op) {
	case MONO_CEE_CONV_OVF_I1_UN: return MONO_CEE_CONV_OVF_I1;
	case MONO_CEE_CONV_OVF_I2_UN: return MONO_CEE_CONV_OVF_I2;
	case MONO_CEE_CONV_OVF_I4_UN: return MONO_CEE_CONV_OVF_I4;
	case MONO_CEE_CONV_OVF_I8_UN: return MONO_CEE_CONV_OVF_I8;
	case MONO_CEE_CONV_OVF_I_UN: return MONO_CEE_CONV_OVF_I;
		   ...
		   guint inline_offset, gboolean emit_check_this)
{
	...
	for (guchar *next_ip = ip; ip < end; ip = next_ip) {
		 ...
		 * the statement around it. That is what the LLVM tier reports for
		 * the same frame.
		...
		case MONO_CEE_CALLVIRT: {
			...
			*/
			...
			/* Not under MONO_OPT_INTRINS: UnsafeMov's own IL boxes its
			 * argument and unboxes it as the return type, which throws
			 * whenever the two types differ. */
			if ((ins = mini_emit_inst_for_sharable_method (cfg, cmethod, fsig, sp))) {
				if (!MONO_TYPE_IS_VOID (fsig->ret)) {
...
calli_end:
		...
		case MONO_CEE_CONV_OVF_I_UN:
			if (sp [-1]->type == STACK_R8 || sp [-1]->type == STACK_R4) {
				...
				// signed, and narrowing it as unsigned would throw on every
				// negative source.
				ADD_UNOP (CEE_CONV_OVF_I8);
				ADD_UNOP (conv_ovf_dropping_un (il_op));
			} else {
				ADD_UNOP (il_op);
			}
			break;
		...
		case MONO_CEE_STSFLD: {
			...
			*/
			 ...
			 * class, an accessor in the field's own class - is really the same
			 * argument: the accessor is only reached through a call to it, and
			 * that call already triggered the cctor. Folding the accessor into a
			 * caller is exactly what removes that call, so the barrier has to be
			 * here, on the path that reaches the field.
			 *
			 * A special static needs the barrier in every compile. The inlined
			 * TLS read below and mono_get_special_static_data () reach one by
			 * its offset alone. Neither carries a vtable, so neither runs the
			 * declaring class's initializer.
			 *
			 * Recorded in class_inits so the check further down does not emit a
			 * second one for the same class in this block.
			 */
			if ((is_special_static || (cfg->llvm_ir_only && !context_used)) &&
