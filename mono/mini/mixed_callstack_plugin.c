...
// mixed_callstack_plugin_save_method_info () below reads the fields of a
// MonoCompile the classic tier-0 compiler produces. mini.h only forward-
// declares that type. The full definition is tier0/compile.h.
#ifdef MONO_ENABLE_TIER0_CLASSIC
#include "tier0/compile.h"
#endif
...
mixed_callstack_plugin_save_method_info (MonoCompile *cfg)
{
	...
	if (seq_points)
	{
		...
		bytes = snprintf(frame, sizeof(frame), "---%p;%p;[%s] %s\n", cfg->native_code, ((char*)cfg->native_code) + cfg->code_size, m_class_get_image (cfg->method->klass)->module_name, method_name);
		...
		while (mono_seq_point_iterator_next(&it))
		{
			...
			bytes = snprintf(frame, sizeof(frame), "%p;%p;[%s] %s : %d;%s\n", cfg->native_code + it.seq_point.native_offset, cfg->native_code + it.seq_point.native_offset, m_class_get_image (cfg->method->klass)->module_name, method_name, location->row, location->source_file);
...
#else // MONO_ENABLE_TIER0_CLASSIC

// The LLVM backend produces neither a MonoCompile nor the classic per-method
// fields above, and never calls this.
void
mixed_callstack_plugin_save_method_info (MonoCompile *cfg)
{
}

#endif // MONO_ENABLE_TIER0_CLASSIC
