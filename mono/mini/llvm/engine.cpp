 ...
 * and this pass recovers the ranges between them, scheduled after every pass that
 * can move code.
 *
...
char g_mono_finally_range_pass_id = 0;
...
class MonoFinallyRangePass : public MachineFunctionPass {
public:
	explicit MonoFinallyRangePass (MonoEHSideChannel &sc)
		: MachineFunctionPass (g_mono_finally_range_pass_id), sc_ (sc)
	{
	}

	StringRef getPassName () const override { return "Mono finally body ranges"; }
...
private:
	 ...
	 */
	static void solve (MachineFunction &mf, int clause,
	                   DenseMap<const MachineBasicBlock *, bool> &in_body)
	{
	...
	static bool transfer (MachineBasicBlock &mbb, int clause, bool in)
	{
		bool state = in;
	 ...
	 */
	void record_ranges (MachineFunction &mf, int clause,
	...
	static MCSymbol *plant_label (MachineBasicBlock &mbb, MachineBasicBlock::iterator at,
	                              MCContext &ctx, const TargetInstrInfo *tii,
	                              const char *name)
	{
	...
	static void close_range (MonoEHFinallyFunction &fn, MachineBasicBlock &mbb,
	                         MachineBasicBlock::iterator at, MCContext &ctx,
	                         const TargetInstrInfo *tii, MCSymbol *begin, int clause)
	{
