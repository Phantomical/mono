...
process_bb (EmitContext *ctx, MonoBasicBlock *bb)
{
	...
	llvm::IRBuilder<> *builder, *starting_builder;
	bool has_terminator;
	LLVMValueRef v;
	...
	for (ins = bb->code; ins; ins = ins->next) {
		...
		LLVMInsertIntoBuilder (llvm::wrap (builder), values [ins->dreg]);
	...
	for (ins = bb->code; ins; ins = ins->next) {
		...
		if (nins > 1000) {
			...
			llvm::wrap (builder->CreateCondBr (llvm::unwrap (cmp), llvm::unwrap (cbb), llvm::unwrap (dummy_bb)));
		...
		case OP_LIVERANGE_END:
			break;
		case OP_ICONST:
			values [ins->dreg] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_c0, false));
			break;
		case OP_I8CONST:
#if TARGET_SIZEOF_VOID_P == 4
			values [ins->dreg] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), get_long_imm (ins), false));
#else
			values [ins->dreg] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), static_cast<gint64>(ins->inst_c0), false));
#endif
			break;
		case OP_R8CONST:
		...
		case OP_DUMMY_ICONST:
			values [ins->dreg] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false));
			break;
		case OP_DUMMY_I8CONST:
			values [ins->dreg] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), 0, false));
			break;
		case OP_DUMMY_R8CONST:
			values [ins->dreg] = llvm::wrap (llvm::ConstantFP::get (llvm::Type::getDoubleTy (ctx->llvm_ctx ()), 0.0f));
			break;
		case OP_BR: {
		...
		case OP_SWITCH: {
			...
			lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())));
			v = llvm::wrap (builder->CreateSwitch (llvm::unwrap (lhs), llvm::unwrap (new_bb), GPOINTER_TO_UINT (ins->klass)));
			...
			new_builder->SetInsertPoint (llvm::unwrap (new_bb));
			llvm::wrap (new_builder->CreateUnreachable ());
		...
		case OP_SETRET:
			...
			case LLVMArgVtypeInReg: {
				...
				retval = llvm::wrap (llvm::UndefValue::get (llvm::unwrap (ret_type)));
				...
				if (!addresses [ins->sreg1]) {
					...
					val = llvm::wrap (builder->CreateBitCast (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (llvm::wrap (llvm::FixedVectorType::get (llvm::unwrap (IntPtrType ()), 2))), ""));

					for (i = 0; i < 2; ++i) {
						if (linfo->ret.pair_storage [i] == LLVMArgInIReg) {
							retval = llvm::wrap (builder->CreateInsertValue (llvm::unwrap (retval), llvm::unwrap (llvm::wrap (builder->CreateExtractElement (llvm::unwrap (val), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), i, false))), ""))), {(unsigned) i}, ""));
						} else {
							g_assert (linfo->ret.pair_storage [i] == LLVMArgNone);
						}
					}
				} else {
					addr = llvm::wrap (builder->CreateBitCast (llvm::unwrap (addresses [ins->sreg1]->value), llvm::unwrap (llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), ""));
					for (i = 0; i < 2; ++i) {
						if (linfo->ret.pair_storage [i] == LLVMArgInIReg) {
							...
							part_addr = llvm::wrap (builder->CreateGEP (llvm::unwrap (ret_type), llvm::unwrap (addr), gep_index_list (indexes, 2), ""));

							retval = llvm::wrap (builder->CreateInsertValue (llvm::unwrap (retval), llvm::unwrap (llvm::wrap (builder->CreateLoad (llvm::unwrap (LLVMStructGetTypeAtIndex (ret_type, i)), llvm::unwrap (part_addr), ""))), {(unsigned) i}, ""));
						} else {
							g_assert (linfo->ret.pair_storage [i] == LLVMArgNone);
						}
					}
			...
			case LLVMArgVtypeAsScalar: {
				...
				if (MONO_CLASS_IS_SIMD (ctx->cfg, mono_class_from_mono_type_internal (sig->ret)))
					retval = llvm::wrap (builder->CreateBitCast (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (ret_type), "setret_cvt_simd"));
				else {
					g_assert (addresses [ins->sreg1]);
					retval = llvm::wrap (builder->CreateLoad (llvm::unwrap (ret_type), llvm::unwrap (llvm::wrap (builder->CreateBitCast (llvm::unwrap (addresses [ins->sreg1]->value), llvm::unwrap (llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), ""))), ""));
				}
				llvm::wrap (builder->CreateRet (llvm::unwrap (retval)));
				break;
			}
			...
			case LLVMArgGsharedvtFixed: {
				...
				if (lhs) {
					...
					llvm::wrap (builder->CreateStore (llvm::unwrap (convert (ctx, lhs, ret_type)), llvm::unwrap (convert (ctx, values [cfg->vret_addr->dreg], llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))))));
				}
				llvm::wrap (builder->CreateRetVoid ());
				break;
			}
			...
			case LLVMArgFpStruct: {
				...
				g_assert (addresses [ins->sreg1]);
				retval = llvm::wrap (builder->CreateLoad (llvm::unwrap (ret_type), llvm::unwrap (convert (ctx, addresses [ins->sreg1]->value, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)))), ""));
				llvm::wrap (builder->CreateRet (llvm::unwrap (retval)));
				break;
			}
			case LLVMArgNone:
			case LLVMArgNormal: {
				if (!lhs || ctx->is_dead [ins->sreg1]) {
					...
					if (cfg->vret_addr)
						...
						llvm::wrap (builder->CreateRet (llvm::unwrap (llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (type_to_llvm_type (ctx, sig->ret)))))));
				} else {
					llvm::wrap (builder->CreateRet (llvm::unwrap (convert (ctx, lhs, type_to_llvm_type (ctx, sig->ret)))));
				}
				has_terminator = true;
				break;
			}
		...
		case OP_COMPARE_IMM: {
			...
			if (ins->opcode == OP_ICOMPARE_IMM) {
				lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())));
				rhs = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_imm, false));
			}
			if (ins->opcode == OP_LCOMPARE_IMM) {
				lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())));
				rhs = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), get_long_imm (ins), false));
			}
			if (ins->opcode == OP_LCOMPARE) {
				lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())));
				rhs = convert (ctx, rhs, llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())));
			}
			if (ins->opcode == OP_ICOMPARE) {
				lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())));
				rhs = convert (ctx, rhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())));
			}
			...
			if (ins->opcode == OP_FCOMPARE) {
				...
				if (LLVMGetTypeKind (LLVMTypeOf (lhs)) == LLVMPointerTypeKind && ins->inst_imm == 0) {
					...
					if (nonnull && llvm_pred == LLVMIntEQ)
						cmp = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt1Ty (ctx->llvm_ctx ()), FALSE, false));
			...
			if (MONO_IS_COND_BRANCH_OP (ins->next)) {
				if (ins->next->inst_true_bb == ins->next->inst_false_bb) {
					 ...
					 */
					llvm::wrap (builder->CreateBr (llvm::unwrap (get_bb (ctx, ins->next->inst_true_bb))));
				} else {
					llvm::wrap (builder->CreateCondBr (llvm::unwrap (cmp), llvm::unwrap (get_bb (ctx, ins->next->inst_true_bb)), llvm::unwrap (get_bb (ctx, ins->next->inst_false_bb))));
				}
				has_terminator = true;
			} else if (MONO_IS_SETCC (ins->next)) {
				sprintf (dname_buf, "t%d", ins->next->dreg);
				dname = dname_buf;
				values [ins->next->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (cmp), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
		...
		case OP_FCLE: {
			...
			cmp = llvm::wrap (builder->CreateFCmp (to_llvm_pred (fpcond_to_llvm_cond [rel]), llvm::unwrap (convert (ctx, lhs, llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())))), llvm::unwrap (convert (ctx, rhs, llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())))), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (cmp), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
			break;
		}
		case OP_RCEQ:
		case OP_RCNEQ:
		...
		case OP_RMOVE: {
			...
			if (var && m_class_get_byval_arg (var->klass)->type == MONO_TYPE_R4) {
				 ...
				 */
				values [ins->dreg] = convert (ctx, values [ins->dreg], llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
			}
			break;
		}
		case OP_MOVE_F_TO_I4: {
			values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (llvm::wrap (builder->CreateFPTrunc (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ()))), ""))), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), ""));
			break;
		}
		case OP_MOVE_I4_TO_F: {
			values [ins->dreg] = llvm::wrap (builder->CreateFPExt (llvm::unwrap (llvm::wrap (builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ()))), ""))), llvm::unwrap (llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()))), ""));
			break;
		}
		case OP_MOVE_F_TO_I8: {
			values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))), ""));
			break;
		}
		case OP_MOVE_I8_TO_F: {
			values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()))), ""));
			break;
		}
		case OP_IADD:
		case OP_ISUB:
		...
		case OP_LSHR_UN:
			...
			case OP_IADD:
			case OP_LADD:
				values [ins->dreg] = llvm::wrap (builder->CreateAdd (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_ISUB:
			case OP_LSUB:
				values [ins->dreg] = llvm::wrap (builder->CreateSub (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_IMUL:
			case OP_LMUL:
				values [ins->dreg] = llvm::wrap (builder->CreateMul (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_IREM:
			case OP_LREM:
				values [ins->dreg] = llvm::wrap (builder->CreateSRem (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_IREM_UN:
			case OP_LREM_UN:
				values [ins->dreg] = llvm::wrap (builder->CreateURem (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_IDIV:
			case OP_LDIV:
				values [ins->dreg] = llvm::wrap (builder->CreateSDiv (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_IDIV_UN:
			case OP_LDIV_UN:
				values [ins->dreg] = llvm::wrap (builder->CreateUDiv (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_FDIV:
			case OP_RDIV:
				values [ins->dreg] = llvm::wrap (builder->CreateFDiv (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_IAND:
			case OP_LAND:
				values [ins->dreg] = llvm::wrap (builder->CreateAnd (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_IOR:
			case OP_LOR:
				values [ins->dreg] = llvm::wrap (builder->CreateOr (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_IXOR:
			case OP_LXOR:
				values [ins->dreg] = llvm::wrap (builder->CreateXor (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_ISHL:
			case OP_LSHL:
				values [ins->dreg] = llvm::wrap (builder->CreateShl (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_ISHR:
			case OP_LSHR:
				values [ins->dreg] = llvm::wrap (builder->CreateAShr (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_ISHR_UN:
			case OP_LSHR_UN:
				values [ins->dreg] = llvm::wrap (builder->CreateLShr (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;

			case OP_FADD:
				values [ins->dreg] = llvm::wrap (builder->CreateFAdd (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_FSUB:
				values [ins->dreg] = llvm::wrap (builder->CreateFSub (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
			case OP_FMUL:
				values [ins->dreg] = llvm::wrap (builder->CreateFMul (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
		...
		case OP_RMUL:
		case OP_RDIV: {
			lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
			rhs = convert (ctx, rhs, llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
			switch (ins->opcode) {
			case OP_RADD:
				values [ins->dreg] = llvm::wrap (builder->CreateFAdd (llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
				break;
		...
		case OP_SHR_UN_IMM: {
			...
			if (spec [MONO_INST_SRC1] == 'l') {
				imm = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), get_long_imm (ins), false));
			} else {
				imm = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_imm, false));
			}
			...
			case OP_LADD_IMM:
			case OP_ADD_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateAdd (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_ISUB_IMM:
			case OP_LSUB_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateSub (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_IMUL_IMM:
			case OP_MUL_IMM:
			case OP_LMUL_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateMul (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_IDIV_IMM:
			case OP_LDIV_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateSDiv (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_IDIV_UN_IMM:
			case OP_LDIV_UN_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateUDiv (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_IREM_IMM:
			case OP_LREM_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateSRem (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_IREM_UN_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateURem (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_IAND_IMM:
			case OP_LAND_IMM:
			case OP_AND_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateAnd (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_IOR_IMM:
			case OP_LOR_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateOr (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_IXOR_IMM:
			case OP_LXOR_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateXor (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_ISHL_IMM:
			case OP_LSHL_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateShl (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_SHL_IMM:
				if (TARGET_SIZEOF_VOID_P == 8) {
					/* The IL is not regular */
					lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())));
					imm = convert (ctx, imm, llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())));
				}
				values [ins->dreg] = llvm::wrap (builder->CreateShl (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_ISHR_IMM:
			case OP_LSHR_IMM:
			case OP_SHR_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateAShr (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_ISHR_UN_IMM:
				/* This is used to implement conv.u4, so the lhs could be an i8 */
				lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())));
				imm = convert (ctx, imm, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())));
				values [ins->dreg] = llvm::wrap (builder->CreateLShr (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
				break;
			case OP_LSHR_UN_IMM:
			case OP_SHR_UN_IMM:
				values [ins->dreg] = llvm::wrap (builder->CreateLShr (llvm::unwrap (lhs), llvm::unwrap (imm), dname));
		...
		case OP_INEG:
			values [ins->dreg] = llvm::wrap (builder->CreateSub (llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), llvm::unwrap (convert (ctx, lhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())))), dname));
			break;
		case OP_LNEG:
			if (LLVMTypeOf (lhs) != llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())))
				lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())));
			values [ins->dreg] = llvm::wrap (builder->CreateSub (llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), 0, false))), llvm::unwrap (lhs), dname));
			break;
		case OP_FNEG:
			lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())));
			values [ins->dreg] = llvm::wrap (builder->CreateFNeg (llvm::unwrap (lhs), dname));
			break;
		case OP_RNEG:
			lhs = convert (ctx, lhs, llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
			values [ins->dreg] = llvm::wrap (builder->CreateFNeg (llvm::unwrap (lhs), dname));
		...
		case OP_INOT: {
			...
			values [ins->dreg] = llvm::wrap (builder->CreateXor (llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), v, false))), llvm::unwrap (convert (ctx, lhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())))), dname));
			break;
		}
		case OP_LNOT: {
			if (LLVMTypeOf (lhs) != llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())))
			...
			values [ins->dreg] = llvm::wrap (builder->CreateXor (llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), v, false))), llvm::unwrap (lhs), dname));
			break;
		}
		...
		case OP_X86_LEA: {
			...
			rhs = llvm::wrap (builder->CreateSExt (llvm::unwrap (convert (ctx, rhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))), ""));

			v1 = llvm::wrap (builder->CreateMul (llvm::unwrap (convert (ctx, rhs, IntPtrType ())), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ((unsigned long long)1 << ins->backend.shift_amount), false))), ""));
			v2 = llvm::wrap (builder->CreateAdd (llvm::unwrap (convert (ctx, lhs, IntPtrType ())), llvm::unwrap (v1), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateAdd (llvm::unwrap (v2), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ins->inst_imm, false))), dname));
			break;
		}
		case OP_X86_BSF32:
		case OP_X86_BSF64: {
		...
		case OP_X86_BSR64: {
			...
			values [ins->dreg] = llvm::wrap (builder->CreateXor (llvm::unwrap (tz), llvm::unwrap (width), dname));
			break;
		}
#endif
		...
		case OP_LCONV_TO_U4: {
			...
			v = llvm::wrap (builder->CreateTrunc (llvm::unwrap (lhs), llvm::unwrap (op_to_llvm_type (ins->opcode)), ""));
			if (sign)
				values [ins->dreg] = llvm::wrap (builder->CreateSExt (llvm::unwrap (v), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
			else
				values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (v), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
			break;
		}
		case OP_ICONV_TO_I8:
			values [ins->dreg] = llvm::wrap (builder->CreateSExt (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))), dname));
			break;
		case OP_ICONV_TO_U8:
			values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))), dname));
			break;
		case OP_FCONV_TO_I4:
		case OP_RCONV_TO_I4:
			values [ins->dreg] = llvm::wrap (builder->CreateFPToSI (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
			break;
		case OP_FCONV_TO_I1:
		case OP_RCONV_TO_I1:
			values [ins->dreg] = llvm::wrap (builder->CreateSExt (llvm::unwrap (llvm::wrap (builder->CreateFPToSI (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt8Ty (ctx->llvm_ctx ()))), dname))), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), ""));
			break;
		case OP_FCONV_TO_U1:
		case OP_RCONV_TO_U1:
			values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (llvm::wrap (builder->CreateTrunc (llvm::unwrap (llvm::wrap (builder->CreateFPToUI (llvm::unwrap (lhs), llvm::unwrap (IntPtrType ()), dname))), llvm::unwrap (llvm::wrap (llvm::Type::getInt8Ty (ctx->llvm_ctx ()))), ""))), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), ""));
			break;
		case OP_FCONV_TO_I2:
		case OP_RCONV_TO_I2:
			values [ins->dreg] = llvm::wrap (builder->CreateSExt (llvm::unwrap (llvm::wrap (builder->CreateFPToSI (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt16Ty (ctx->llvm_ctx ()))), dname))), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), ""));
			break;
		case OP_FCONV_TO_U2:
		case OP_RCONV_TO_U2:
			values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (llvm::wrap (builder->CreateFPToUI (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt16Ty (ctx->llvm_ctx ()))), dname))), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), ""));
			break;
		case OP_FCONV_TO_U4:
		case OP_RCONV_TO_U4:
			values [ins->dreg] = llvm::wrap (builder->CreateFPToUI (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
			break;
		case OP_FCONV_TO_U8:
		case OP_RCONV_TO_U8:
			values [ins->dreg] = llvm::wrap (builder->CreateFPToUI (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))), dname));
			break;
		case OP_FCONV_TO_I8:
		case OP_RCONV_TO_I8:
			values [ins->dreg] = llvm::wrap (builder->CreateFPToSI (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))), dname));
			break;
		case OP_FCONV_TO_I:
			values [ins->dreg] = llvm::wrap (builder->CreateFPToSI (llvm::unwrap (lhs), llvm::unwrap (IntPtrType ()), dname));
			break;
		case OP_ICONV_TO_R8:
		case OP_LCONV_TO_R8:
			values [ins->dreg] = llvm::wrap (builder->CreateSIToFP (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()))), dname));
			break;
		case OP_ICONV_TO_R_UN:
		case OP_LCONV_TO_R_UN:
			values [ins->dreg] = llvm::wrap (builder->CreateUIToFP (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()))), dname));
			break;
#if TARGET_SIZEOF_VOID_P == 4
		case OP_LCONV_TO_U:
#endif
		case OP_LCONV_TO_I4:
			values [ins->dreg] = llvm::wrap (builder->CreateTrunc (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
			break;
		case OP_ICONV_TO_R4:
		case OP_LCONV_TO_R4:
			v = llvm::wrap (builder->CreateSIToFP (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ()))), ""));
			if (cfg->r4fp)
				values [ins->dreg] = v;
			else
				values [ins->dreg] = llvm::wrap (builder->CreateFPExt (llvm::unwrap (v), llvm::unwrap (llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()))), dname));
			break;
		case OP_FCONV_TO_R4:
			v = llvm::wrap (builder->CreateFPTrunc (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ()))), ""));
			if (cfg->r4fp)
				values [ins->dreg] = v;
			else
				values [ins->dreg] = llvm::wrap (builder->CreateFPExt (llvm::unwrap (v), llvm::unwrap (llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()))), dname));
			break;
		case OP_RCONV_TO_R8:
			values [ins->dreg] = llvm::wrap (builder->CreateFPExt (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()))), dname));
			break;
		case OP_RCONV_TO_R4:
			values [ins->dreg] = lhs;
			break;
		case OP_SEXT_I4:
			values [ins->dreg] = llvm::wrap (builder->CreateSExt (llvm::unwrap (convert (ctx, lhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))), dname));
			break;
		case OP_ZEXT_I4:
			values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (convert (ctx, lhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))), dname));
			break;
		case OP_TRUNC_I4:
			values [ins->dreg] = llvm::wrap (builder->CreateTrunc (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
			break;
		...
		case OP_LOCALLOC: {
			...
			size = llvm::wrap (builder->CreateAnd (llvm::unwrap (llvm::wrap (builder->CreateAdd (llvm::unwrap (convert (ctx, lhs, llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), MONO_ARCH_FRAME_ALIGNMENT - 1, false))), ""))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ~ (MONO_ARCH_FRAME_ALIGNMENT - 1), false))), ""));
		...
		case OP_LOAD_MEM: {
			...
			if ((ins->opcode == OP_LOADI8_MEM) || (ins->opcode == OP_LOAD_MEM) || (ins->opcode == OP_LOADI4_MEM) || (ins->opcode == OP_LOADU4_MEM) || (ins->opcode == OP_LOADU1_MEM) || (ins->opcode == OP_LOADU2_MEM)) {
				addr = llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ins->inst_imm, false));
				base = addr;
			} else {
				/* _MEMBASE */
				base = lhs;
				...
				if (ins->inst_offset == 0) {
					...
					if (mono_llvm_can_be_gep (base, &gep_base, &gep_offset)) {
						addr = llvm::wrap (builder->CreateGEP (llvm::unwrap (llvm::wrap (llvm::Type::getInt8Ty (ctx->llvm_ctx ()))), llvm::unwrap (convert (ctx, gep_base, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)))), gep_index_list (&gep_offset, 1), ""));
					} else {
						addr = base;
					}
				} else if (ins->inst_offset % size != 0) {
					/* Unaligned load */
					index = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_offset, false));
					addr = llvm::wrap (builder->CreateGEP (llvm::unwrap (llvm::wrap (llvm::Type::getInt8Ty (ctx->llvm_ctx ()))), llvm::unwrap (convert (ctx, base, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)))), gep_index_list (&index, 1), ""));
				} else {
					index = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_offset / size, false));
					addr = llvm::wrap (builder->CreateGEP (llvm::unwrap (t), llvm::unwrap (convert (ctx, base, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)))), gep_index_list (&index, 1), ""));
				}
			}

			addr = convert (ctx, addr, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));

			if (is_unaligned)
				values [ins->dreg] = mono_llvm_build_aligned_load (llvm::wrap (builder), t, addr, dname, is_volatile, 1);
			else
			...
			if (sext)
				values [ins->dreg] = llvm::wrap (builder->CreateSExt (llvm::unwrap (values [ins->dreg]), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
			else if (zext)
				values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (values [ins->dreg]), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
			else if (!cfg->r4fp && ins->opcode == OP_LOADR4_MEMBASE)
				values [ins->dreg] = llvm::wrap (builder->CreateFPExt (llvm::unwrap (values [ins->dreg]), llvm::unwrap (llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()))), dname));
			break;
		}
		...
		case OP_STORE_MEMBASE_REG: {
			...
			LLVMValueRef ptrdst = convert (ctx, addr, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
		...
		case OP_STORE_MEMBASE_IMM: {
			...
			LLVMValueRef srcval = convert (ctx, llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ins->inst_imm, false)), t);
			LLVMValueRef ptrdst = convert (ctx, addr, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			if (is_unaligned)
				mono_llvm_build_aligned_store (llvm::wrap (builder), srcval, ptrdst, is_volatile, 1);
			else
		...
		case OP_CHECK_THIS:
			emit_load (ctx, bb, &builder, TARGET_SIZEOF_VOID_P, IntPtrType (), convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), lhs, "", TRUE, FALSE, LLVM_BARRIER_NONE);
			break;
		...
		case OP_AOTCONST: {
			...
			if (ji_type == MONO_PATCH_INFO_ICALL_ADDR) {
				...
				if (symbol) {
					 ...
					 */
					g_free (symbol);
					values [ins->dreg] = llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), 0, false));
					break;
				}
			}

			values [ins->dreg] = get_aotconst (ctx, ji_type, ji_data, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			break;
		}
		case OP_MEMMOVE: {
			int argn = 0;
			LLVMValueRef args [5];
			args [argn++] = convert (ctx, values [ins->sreg1], llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			args [argn++] = convert (ctx, values [ins->sreg2], llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			args [argn++] = convert (ctx, values [ins->sreg3], llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())));
#if LLVM_API_VERSION < 900
			args [argn++] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 1, false)); // alignment
#endif
		...
		case OP_NOT_REACHED:
			...
			while (ins->next) {
				...
				if (spec [MONO_INST_DEST] == 'i' && !MONO_IS_STORE_MEMBASE (next))
					ctx->values [next->dreg] = llvm::wrap (llvm::Constant::getNullValue (llvm::Type::getInt32Ty (ctx->llvm_ctx ())));
		...
		case OP_SIN: {
			...
			args [0] = convert (ctx, lhs, llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())));
		...
		case OP_SINF: {
			...
			args [0] = convert (ctx, lhs, llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
		...
		case OP_FMA: {
			...
			args [0] = convert (ctx, values [ins->sreg1], llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())));
			args [1] = convert (ctx, values [ins->sreg2], llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())));
			args [2] = convert (ctx, values [ins->sreg3], llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())));
		...
		case OP_FMAF: {
			...
			args [0] = convert (ctx, values [ins->sreg1], llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
			args [1] = convert (ctx, values [ins->sreg2], llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
			args [2] = convert (ctx, values [ins->sreg3], llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
		...
		case OP_RPOW: {
			...
			args [0] = convert (ctx, lhs, llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
			args [1] = convert (ctx, rhs, llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())));
		...
		case OP_FPOW: {
			...
			args [0] = convert (ctx, lhs, llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())));
			args [1] = convert (ctx, rhs, llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())));
		...
		case OP_RMAX: {
			...
			values [ins->dreg] = llvm::wrap (builder->CreateSelect (llvm::unwrap (v), llvm::unwrap (lhs), llvm::unwrap (rhs), dname));
			break;
		}
 ...
 */
#ifdef TARGET_ARM64
#define ARM64_ATOMIC_FENCE_FIX mono_llvm_build_fence (llvm::wrap (builder), LLVM_BARRIER_SEQ)
#else
#define ARM64_ATOMIC_FENCE_FIX
		...
		case OP_ATOMIC_EXCHANGE_I8: {
			...
			if (ins->opcode == OP_ATOMIC_EXCHANGE_I4)
				t = llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()));
			else
				t = llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()));

			g_assert (ins->inst_offset == 0);

			args [0] = convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			args [1] = convert (ctx, rhs, t);

			ARM64_ATOMIC_FENCE_FIX;
			values [ins->dreg] = mono_llvm_build_atomic_rmw (llvm::wrap (builder), LLVM_ATOMICRMW_OP_XCHG, args [0], args [1]);
			ARM64_ATOMIC_FENCE_FIX;
			break;
		}
		...
		case OP_ATOMIC_OR_I8: {
			...
			if (ins->opcode == OP_ATOMIC_ADD_I4 || ins->opcode == OP_ATOMIC_ADD_I8)
				...
				values [ins->dreg] = llvm::wrap (builder->CreateAdd (llvm::unwrap (mono_llvm_build_atomic_rmw (llvm::wrap (builder), LLVM_ATOMICRMW_OP_ADD, args [0], args [1])), llvm::unwrap (args [1]), dname));
			else if (ins->opcode == OP_ATOMIC_AND_I4 || ins->opcode == OP_ATOMIC_AND_I8)
				values [ins->dreg] = mono_llvm_build_atomic_rmw (llvm::wrap (builder), LLVM_ATOMICRMW_OP_AND, args [0], args [1]);
			else if (ins->opcode == OP_ATOMIC_OR_I4 || ins->opcode == OP_ATOMIC_OR_I8)
				values [ins->dreg] = mono_llvm_build_atomic_rmw (llvm::wrap (builder), LLVM_ATOMICRMW_OP_OR, args [0], args [1]);
			else
				g_assert_not_reached ();
			ARM64_ATOMIC_FENCE_FIX;
			break;
		...
		case OP_ATOMIC_CAS_I8: {
			...
			ARM64_ATOMIC_FENCE_FIX;
			val = mono_llvm_build_cmpxchg (llvm::wrap (builder), args [0], args [1], args [2]);
			ARM64_ATOMIC_FENCE_FIX;
			/* cmpxchg returns a pair */
			values [ins->dreg] = llvm::wrap (builder->CreateExtractValue (llvm::unwrap (val), {0}, ""));
			break;
		}
		case OP_MEMORY_BARRIER: {
			mono_llvm_build_fence (llvm::wrap (builder), static_cast<BarrierKind>(ins->backend.memory_barrier_kind));
			break;
		}
		...
		case OP_ATOMIC_LOAD_R8: {
			...
			if (ins->inst_offset != 0) {
				index = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_offset / size, false));
				addr = llvm::wrap (builder->CreateGEP (llvm::unwrap (t), llvm::unwrap (convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)))), gep_index_list (&index, 1), ""));
			} else {
				addr = lhs;
			}
		...
		case OP_TLS_GET: {
			...
			LLVMTypeRef ptrtype = llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 257));
#else
			// 256 == GS segment register
			LLVMTypeRef ptrtype = llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 256));
#endif
			// FIXME: XEN
			values [ins->dreg] = llvm::wrap (builder->CreateLoad (llvm::unwrap (IntPtrType ()), llvm::unwrap (llvm::wrap (builder->CreateIntToPtr (llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ins->inst_offset, true))), llvm::unwrap (ptrtype), ""))), ""));
			...
			// 256 == GS segment register
			LLVMTypeRef ptrtype = llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 256));
			values [ins->dreg] = llvm::wrap (builder->CreateLoad (llvm::unwrap (IntPtrType ()), llvm::unwrap (llvm::wrap (builder->CreateIntToPtr (llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), offset, true))), llvm::unwrap (ptrtype), ""))), ""));
#else
		...
		case OP_LMUL_OVF_UN: {
			...
			values [ins->dreg] = llvm::wrap (builder->CreateExtractValue (llvm::unwrap (val), {0}, dname));
			ovf = llvm::wrap (builder->CreateExtractValue (llvm::unwrap (val), {1}, ""));
		...
		case OP_VZERO: {
			...
			LLVMValueRef ptr = llvm::wrap (builder->CreateBitCast (llvm::unwrap (addresses [ins->dreg]->value), llvm::unwrap (llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), ""));
		...
		case OP_VMOVE: {
			...
			case OP_STOREV_MEMBASE:
				...
				if (!addresses [ins->sreg1]) {
					...
					dst = convert (ctx, llvm::wrap (builder->CreateAdd (llvm::unwrap (convert (ctx, values [ins->inst_destbasereg], IntPtrType ())), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ins->inst_offset, false))), "")), llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
					llvm::wrap (builder->CreateStore (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (dst)));
					done = true;
				} else {
					src = llvm::wrap (builder->CreateBitCast (llvm::unwrap (addresses [ins->sreg1]->value), llvm::unwrap (llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), ""));
					dst = convert (ctx, llvm::wrap (builder->CreateAdd (llvm::unwrap (convert (ctx, values [ins->inst_destbasereg], IntPtrType ())), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ins->inst_offset, false))), "")), llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
				}
				break;
			case OP_LOADV_MEMBASE:
				...
				src = convert (ctx, llvm::wrap (builder->CreateAdd (llvm::unwrap (convert (ctx, values [ins->inst_basereg], IntPtrType ())), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ins->inst_offset, false))), "")), llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
				dst = llvm::wrap (builder->CreateBitCast (llvm::unwrap (addresses [ins->dreg]->value), llvm::unwrap (llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), ""));
				break;
			case OP_VMOVE:
				if (!addresses [ins->sreg1])
			...
			args [aindex ++] = dst;
			args [aindex ++] = src;
			args [aindex ++] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), mono_class_value_size (klass, NULL), false));
#if LLVM_API_VERSION < 900
			// FIXME: Alignment
		...
		case OP_LLVM_OUTARG_VT: {
			...
			if (ainfo->storage == LLVMArgGsharedvtVariable) {
					...
					if (var && var->opcode == OP_GSHAREDVT_LOCAL) {
						addresses [ins->dreg] = create_address (ctx, convert (ctx, emit_gsharedvt_ldaddr (ctx, var->dreg), llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), IntPtrType ());
					} else {
						g_assert (addresses [ins->sreg1]);
				...
				llvm::wrap (builder->CreateStore (llvm::unwrap (convert (ctx, values [ins->sreg1], addresses [ins->sreg1]->type)), llvm::unwrap (addresses [ins->sreg1]->value)));
				addresses [ins->dreg] = addresses [ins->sreg1];
			} else {
				...
				if (!addresses [ins->sreg1]) {
					addresses [ins->sreg1] = build_alloca_address (ctx, t);
					g_assert (values [ins->sreg1]);
					llvm::wrap (builder->CreateStore (llvm::unwrap (convert (ctx, values [ins->sreg1], etype)), llvm::unwrap (addresses [ins->sreg1]->value)));
					addresses [ins->dreg] = addresses [ins->sreg1];
					...
					llvm::wrap (builder->CreateStore (llvm::unwrap (convert (ctx, v, etype)), llvm::unwrap (addresses [ins->dreg]->value)));
				} else {
					addresses [ins->dreg] = addresses [ins->sreg1];
				}
			}
		...
		case OP_OBJC_GET_SELECTOR: {
			...
			if (!var) {
				...
				LLVMValueRef ref_var = LLVMAddGlobal (ctx->lmodule, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)), "@OBJC_SELECTOR_REFERENCES_");

				indexes [0] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false));
				indexes [1] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false));
			...
			values [ins->dreg] = llvm::wrap (builder->CreateLoad (llvm::unwrap (llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), llvm::unwrap (var), ""));
			break;
		}
...
#endif
			for (i = 0; i < 32; ++i)
				mask [i] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false));

			v = convert (ctx, values [ins->sreg1], llvm::wrap (llvm::cast<llvm::VectorType> (llvm::unwrap (t))->getElementType ()));

			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (t)))), llvm::unwrap (v), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (values [ins->dreg]), llvm::unwrap (llvm::wrap (llvm::UndefValue::get (llvm::unwrap (t)))), llvm::unwrap (const_vector (mask, LLVMGetVectorSize (t))), ""));
			break;
		}
		case OP_XZERO: {
			values [ins->dreg] = llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (type_to_llvm_type (ctx, m_class_get_byval_arg (ins->klass)))));
			break;
		}
		case OP_LOADX_MEMBASE: {
			...
			src = convert (ctx, llvm::wrap (builder->CreateAdd (llvm::unwrap (convert (ctx, values [ins->inst_basereg], IntPtrType ())), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ins->inst_offset, false))), "")), llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			values [ins->dreg] = mono_llvm_build_aligned_load (llvm::wrap (builder), t, src, "", FALSE, 1);
			break;
		}
		case OP_STOREX_MEMBASE: {
			LLVMValueRef dest;

			dest = convert (ctx, llvm::wrap (builder->CreateAdd (llvm::unwrap (convert (ctx, values [ins->inst_destbasereg], IntPtrType ())), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (IntPtrType ()), ins->inst_offset, false))), "")), llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			mono_llvm_build_aligned_store (llvm::wrap (builder), values [ins->sreg1], dest, FALSE, 1);
			break;
		}
		...
		case OP_PADDD:
		case OP_PADDQ:
			values [ins->dreg] = llvm::wrap (builder->CreateAdd (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_ADDPD:
		case OP_ADDPS:
			values [ins->dreg] = llvm::wrap (builder->CreateFAdd (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_PSUBB:
		case OP_PSUBW:
		case OP_PSUBD:
		case OP_PSUBQ:
			values [ins->dreg] = llvm::wrap (builder->CreateSub (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_SUBPD:
		case OP_SUBPS:
			values [ins->dreg] = llvm::wrap (builder->CreateFSub (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_MULPD:
		case OP_MULPS:
			values [ins->dreg] = llvm::wrap (builder->CreateFMul (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_DIVPD:
		case OP_DIVPS:
			values [ins->dreg] = llvm::wrap (builder->CreateFDiv (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_PAND:
			values [ins->dreg] = llvm::wrap (builder->CreateAnd (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_POR:
			values [ins->dreg] = llvm::wrap (builder->CreateOr (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_PXOR:
			values [ins->dreg] = llvm::wrap (builder->CreateXor (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_PMULW:
		case OP_PMULD:
			values [ins->dreg] = llvm::wrap (builder->CreateMul (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		case OP_ANDPS:
		case OP_ANDNPS:
		case OP_ORPS:
		...
		case OP_XORPD: {
			...
			case OP_ORPS:
			case OP_XORPS:
				t = llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 4));
				rt = llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getFloatTy (ctx->llvm_ctx ()), 4));
				break;
			case OP_ANDPD:
			case OP_ANDNPD:
			case OP_ORPD:
			case OP_XORPD:
				t = llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), 2));
				rt = llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getDoubleTy (ctx->llvm_ctx ()), 2));
				break;
			default:
				t = llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()));
				rt = llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()));
				g_assert_not_reached ();
			}

			lhs = llvm::wrap (builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (t), ""));
			rhs = llvm::wrap (builder->CreateBitCast (llvm::unwrap (rhs), llvm::unwrap (t), ""));
			switch (ins->opcode) {
			case OP_ANDPS:
			case OP_ANDPD:
				v = llvm::wrap (builder->CreateAnd (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
				break;
			case OP_ORPS:
			case OP_ORPD:
				v = llvm::wrap (builder->CreateOr (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
				break;
			case OP_XORPS:
			case OP_XORPD:
				v = llvm::wrap (builder->CreateXor (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
				break;
			case OP_ANDNPS:
			case OP_ANDNPD:
				v = llvm::wrap (builder->CreateAnd (llvm::unwrap (rhs), llvm::unwrap (llvm::wrap (builder->CreateNot (llvm::unwrap (lhs), ""))), ""));
				break;
			}
			values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (v), llvm::unwrap (rt), ""));
			break;
		}
		case OP_PMIND_UN:
		case OP_PMINW_UN:
		case OP_PMINB_UN: {
			...
			values [ins->dreg] = llvm::wrap (builder->CreateSelect (llvm::unwrap (cmp), llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;
		}
		case OP_PMAXD_UN:
		case OP_PMAXW_UN:
		...
		case OP_PAVGW_UN: {
			...
			LLVMTypeRef ext_elem_type = vector_size == 16 ? llvm::wrap (llvm::Type::getInt16Ty (ctx->llvm_ctx ())) : llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()));

			for (int i = 0; i < 32; ++i)
				ones [i] = llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (ext_elem_type), 1, false));
			...
			val = llvm::wrap (builder->CreateAdd (llvm::unwrap (llvm::wrap (builder->CreateZExt (llvm::unwrap (lhs), llvm::unwrap (ext_type), ""))), llvm::unwrap (llvm::wrap (builder->CreateZExt (llvm::unwrap (rhs), llvm::unwrap (ext_type), ""))), ""));
			val = llvm::wrap (builder->CreateAdd (llvm::unwrap (val), llvm::unwrap (ones_vec), ""));
			val = llvm::wrap (builder->CreateLShr (llvm::unwrap (val), llvm::unwrap (ones_vec), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateTrunc (llvm::unwrap (val), llvm::unwrap (LLVMTypeOf (lhs)), ""));
			break;
		}
		case OP_PCMPEQB:
		case OP_PCMPEQW:
		...
		case OP_PCMPGTB: {
			...
			values [ins->dreg] = llvm::wrap (builder->CreateSExt (llvm::unwrap (pcmp), llvm::unwrap (retType), ""));
			break;
		}
		case OP_EXTRACT_R4:
		case OP_EXTRACT_R8:
		...
		case OP_EXTRACT_U1: {
			...
			lhs = llvm::wrap (builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (t), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_c0, false))), ""));
			if (zext)
				values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (values [ins->dreg]), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), ""));
			break;
		}

		case OP_INSERT_I1:
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (convert (ctx, values [ins->sreg2], llvm::wrap (llvm::Type::getInt8Ty (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_c0, false))), dname));
			break;
		case OP_INSERT_I2:
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (convert (ctx, values [ins->sreg2], llvm::wrap (llvm::Type::getInt16Ty (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_c0, false))), dname));
			break;
		case OP_INSERT_I4:
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (convert (ctx, values [ins->sreg2], llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_c0, false))), dname));
			break;
		case OP_INSERT_I8:
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (convert (ctx, values [ins->sreg2], llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_c0, false))), dname));
			break;
		case OP_INSERT_R4:
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (convert (ctx, values [ins->sreg2], llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_c0, false))), dname));
			break;
		case OP_INSERT_R8:
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (convert (ctx, values [ins->sreg2], llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_c0, false))), dname));
			break;
		case OP_XINSERT_I2: {
			...
			switch_ins = llvm::wrap (builder->CreateSwitch (llvm::unwrap (llvm::wrap (builder->CreateAnd (llvm::unwrap (index), llvm::unwrap (const_int32 (0xf)), ""))), llvm::unwrap (bbs [0]), 0));
			for (i = 0; i < nelems; ++i) {
				...
				phi_values [i] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (vector), llvm::unwrap (convert (ctx, value, llvm::wrap (llvm::cast<llvm::VectorType> (llvm::unwrap (LLVMTypeOf (vector)))->getElementType ()))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), i, false))), ""));
				llvm::wrap (builder->CreateBr (llvm::unwrap (cbb)));
			}

			builder->SetInsertPoint (llvm::unwrap (cbb));
			values [ins->dreg] = llvm::wrap (builder->CreatePHI (llvm::unwrap (LLVMTypeOf (phi_values [0])), 0, ""));
		...
		case OP_CVTDQ2PS: {
			LLVMValueRef i4 = llvm::wrap (builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (sse_i4_t), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateSIToFP (llvm::unwrap (i4), llvm::unwrap (sse_r4_t), dname));
			break;
		}
		case OP_CVTDQ2PD: {
			LLVMValueRef indexes [16];
			...
			indexes [0] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false));
			indexes [1] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 1, false));
			LLVMValueRef mask = const_vector (indexes, 2);
			LLVMValueRef shuffle = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (LLVMTypeOf (lhs))))), llvm::unwrap (mask), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateSIToFP (llvm::unwrap (shuffle), llvm::unwrap (llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getDoubleTy (ctx->llvm_ctx ()), 2))), dname));
			break;
		}
		case OP_SSE2_CVTSS2SD: {
			LLVMValueRef rhs_elem = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (rhs), llvm::unwrap (const_int32 (0)), ""));
			LLVMValueRef fpext = llvm::wrap (builder->CreateFPExt (llvm::unwrap (rhs_elem), llvm::unwrap (llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()))), dname));
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (lhs), llvm::unwrap (fpext), llvm::unwrap (const_int32 (0)), ""));
			break;
		}
		case OP_CVTPS2PD: {
			LLVMValueRef indexes [16];
			...
			LLVMValueRef mask = const_vector (indexes, 2);
			LLVMValueRef shuffle = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (LLVMTypeOf (lhs))))), llvm::unwrap (mask), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateFPExt (llvm::unwrap (shuffle), llvm::unwrap (llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getDoubleTy (ctx->llvm_ctx ()), 2))), dname));
			break;
		}
		case OP_CVTTPS2DQ:
			values [ins->dreg] = llvm::wrap (builder->CreateFPToSI (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 4))), dname));
			break;
		...
		case OP_COMPPD: {
			...
			if (ins->opcode == OP_COMPPD)
				values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (llvm::wrap (builder->CreateSExt (llvm::unwrap (cmp), llvm::unwrap (llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), 2))), ""))), llvm::unwrap (LLVMTypeOf (lhs)), ""));
			else
				values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (llvm::wrap (builder->CreateSExt (llvm::unwrap (cmp), llvm::unwrap (llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 4))), ""))), llvm::unwrap (LLVMTypeOf (lhs)), ""));
			break;
		}
		case OP_ICONV_TO_X:
		...
		case OP_PSHUFLEW_HIGH: {
			...
			case OP_PSHUFLEW_LOW:
				...
				mask [7] = 4 + 3;
				v1 = values [ins->sreg1];
				v2 = llvm::wrap (llvm::UndefValue::get (llvm::unwrap (LLVMTypeOf (v1))));
				break;
			case OP_PSHUFLEW_HIGH:
				mask_size = 8;
				mask [0] = 0;
			...
			for (i = 0; i < mask_size; ++i)
				mask_values [i] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), mask [i], false));

			values [ins->dreg] =
				llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (v1), llvm::unwrap (v2), llvm::unwrap (const_vector (mask_values, mask_size)), dname));
			break;
		}
		...
		case OP_UNPACK_HIGHPD: {
			...
			values [ins->dreg] =
				llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (values [ins->sreg2]), llvm::unwrap (const_vector (mask_values, mask_size)), dname));
			break;
		}
		...
		case OP_DUPPD: {
			...
			v = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			val = llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (t)));
			val = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (val), llvm::unwrap (v), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			val = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (val), llvm::unwrap (v), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 1, false))), dname));
		...
		case OP_DUPPS_HIGH: {
			...
			if (ins->opcode == OP_DUPPS_LOW) {
				v1 = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
				v2 = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 2, false))), ""));
			} else {
				v1 = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 1, false))), ""));
				v2 = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 3, false))), ""));
			}
			val = llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (t)));
			val = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (val), llvm::unwrap (v1), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			val = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (val), llvm::unwrap (v1), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 1, false))), ""));
			val = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (val), llvm::unwrap (v2), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 2, false))), ""));
			val = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (val), llvm::unwrap (v2), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 3, false))), ""));
		...
		case OP_FCONV_TO_R8_X: {
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (sse_r8_t)))), llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			break;
		}

		case OP_FCONV_TO_R4_X: {
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (sse_r4_t)))), llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			break;
		}
		...
		case OP_SSE_MOVS2: {
			if (ins->inst_c1 == MONO_TYPE_R4)
				values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (rhs), llvm::unwrap (lhs), llvm::unwrap (create_const_vector_4_i32 (0, 5, 6, 7)), ""));
			else if (ins->inst_c1 == MONO_TYPE_R8)
				values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (rhs), llvm::unwrap (lhs), llvm::unwrap (create_const_vector_2_i32 (0, 3)), ""));
			else if (ins->inst_c1 == MONO_TYPE_I8 || ins->inst_c1 == MONO_TYPE_U8)
				values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), 0, false))), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 1, false))), ""));
			else
		...
		case OP_SSE_MOVEHL: {
			if (ins->inst_c1 == MONO_TYPE_R4)
				values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (rhs), llvm::unwrap (create_const_vector_4_i32 (6, 7, 2, 3)), ""));
			else
				g_assert_not_reached ();
			break;
		}
		...
		case OP_SSE_MOVELH: {
			if (ins->inst_c1 == MONO_TYPE_R4)
				values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (rhs), llvm::unwrap (create_const_vector_4_i32 (0, 1, 4, 5)), ""));
			else
				g_assert_not_reached ();
			break;
		}
		...
		case OP_SSE_UNPACKLO: {
			if (ins->inst_c1 == MONO_TYPE_R8 || ins->inst_c1 == MONO_TYPE_I8 || ins->inst_c1 == MONO_TYPE_U8) {
				values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (rhs), llvm::unwrap (create_const_vector_2_i32 (0, 2)), ""));
			} else if (ins->inst_c1 == MONO_TYPE_R4 || ins->inst_c1 == MONO_TYPE_I4  || ins->inst_c1 == MONO_TYPE_U4) {
				values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (rhs), llvm::unwrap (create_const_vector_4_i32 (0, 4, 1, 5)), ""));
			} else if (ins->inst_c1 == MONO_TYPE_I2 || ins->inst_c1 == MONO_TYPE_U2) {
				const int mask_values [] = { 0, 8, 1, 9, 2, 10, 3, 11 };
				LLVMValueRef shuffled = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (convert (ctx, lhs, sse_i2_t)), llvm::unwrap (convert (ctx, rhs, sse_i2_t)), llvm::unwrap (create_const_vector_i32 (mask_values, 8)), ""));
				...
				LLVMValueRef shuffled = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (convert (ctx, lhs, sse_i1_t)), llvm::unwrap (convert (ctx, rhs, sse_i1_t)), llvm::unwrap (create_const_vector_i32 (mask_values, 16)), ""));
				values [ins->dreg] = convert (ctx, shuffled, type_to_sse_type (ins->inst_c1));
			} else {
				g_assert_not_reached ();
			}
		...
		case OP_SSE_UNPACKHI: {
			if (ins->inst_c1 == MONO_TYPE_R8 || ins->inst_c1 == MONO_TYPE_I8 || ins->inst_c1 == MONO_TYPE_U8) {
				values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (rhs), llvm::unwrap (create_const_vector_2_i32 (1, 3)), ""));
			} else if (ins->inst_c1 == MONO_TYPE_R4 || ins->inst_c1 == MONO_TYPE_I4  || ins->inst_c1 == MONO_TYPE_U4) {
				values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (rhs), llvm::unwrap (create_const_vector_4_i32 (2, 6, 3, 7)), ""));
			} else if (ins->inst_c1 == MONO_TYPE_I2 || ins->inst_c1 == MONO_TYPE_U2) {
		...
		case OP_SSE_LOADU: {
			LLVMValueRef dst_ptr = convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef dst_vec = llvm::wrap (builder->CreateBitCast (llvm::unwrap (dst_ptr), llvm::unwrap (llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), ""));
			values [ins->dreg] = mono_llvm_build_aligned_load (llvm::wrap (builder), type_to_sse_type (ins->inst_c1), dst_vec, "", FALSE, ins->inst_c0); // inst_c0 is alignment
			break;
		}
		case OP_SSE_MOVSS: {
			LLVMValueRef addr = convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef val = mono_llvm_build_load (llvm::wrap (builder), llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())), addr, "", FALSE);
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (type_to_sse_type (ins->inst_c1))))), llvm::unwrap (val), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			break;
		}
		case OP_SSE_MOVSS_STORE: {
			LLVMValueRef addr = convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef val = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (rhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			mono_llvm_build_store (llvm::wrap (builder), val, addr, FALSE, LLVM_BARRIER_NONE);
			break;
		}
		case OP_SSE2_MOVD:
		case OP_SSE2_MOVQ:
		case OP_SSE2_MOVUPD: {
			...
			LLVMValueRef zero = llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (rty)));
			LLVMValueRef addr = convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef val = mono_llvm_build_aligned_load (llvm::wrap (builder), srcty, addr, "", FALSE, 1);
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (zero), llvm::unwrap (val), llvm::unwrap (const_int32 (0)), dname));
			break;
		}

		case OP_SSE_MOVLPS_LOAD:
		case OP_SSE_MOVHPS_LOAD: {
			LLVMTypeRef t = llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ()));
			int size = 4;
			...
			LLVMValueRef addr1 = convert (ctx, addr, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef addr2 = convert (ctx, llvm::wrap (builder->CreateAdd (llvm::unwrap (convert (ctx, addr, IntPtrType ())), llvm::unwrap (convert (ctx, llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), size, false)), IntPtrType ())), "")), llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef val1 = mono_llvm_build_load (llvm::wrap (builder), t, addr1, "", FALSE);
			LLVMValueRef val2 = mono_llvm_build_load (llvm::wrap (builder), t, addr2, "", FALSE);
			int index1, index2;

			index1 = high ? 2: 0;
			index2 = high ? 3 : 1;
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (llvm::wrap (builder->CreateInsertElement (llvm::unwrap (lhs), llvm::unwrap (val1), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), index1, false))), ""))), llvm::unwrap (val2), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), index2, false))), ""));
			break;
		}

		case OP_SSE2_MOVLPD_LOAD:
		case OP_SSE2_MOVHPD_LOAD: {
			LLVMTypeRef t = llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()));
			LLVMValueRef addr = convert (ctx, rhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef val = mono_llvm_build_load (llvm::wrap (builder), t, addr, "", FALSE);
			int index = ins->opcode == OP_SSE2_MOVHPD_LOAD ? 1 : 0;
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (lhs), llvm::unwrap (val), llvm::unwrap (const_int32 (index)), ""));
			break;
		}
		...
		case OP_SSE_MOVHPS_STORE: {
			...
			LLVMValueRef addr1 = convert (ctx, addr, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef addr2 = convert (ctx, llvm::wrap (builder->CreateAdd (llvm::unwrap (convert (ctx, addr, IntPtrType ())), llvm::unwrap (convert (ctx, llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 4, false)), IntPtrType ())), "")), llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			int index1 = ins->opcode == OP_SSE_MOVLPS_STORE ? 0 : 2;
			int index2 = ins->opcode == OP_SSE_MOVLPS_STORE ? 1 : 3;
			LLVMValueRef val1 = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (rhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), index1, false))), ""));
			LLVMValueRef val2 = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (rhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), index2, false))), ""));
			mono_llvm_build_store (llvm::wrap (builder), val1, addr1, FALSE, LLVM_BARRIER_NONE);
			mono_llvm_build_store (llvm::wrap (builder), val2, addr2, FALSE, LLVM_BARRIER_NONE);
			break;
		}
		...
		case OP_SSE2_MOVHPD_STORE: {
			...
			LLVMValueRef val = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (rhs), llvm::unwrap (const_int32 (index)), ""));
			mono_llvm_build_store (llvm::wrap (builder), val, addr, FALSE, LLVM_BARRIER_NONE);
			break;
		}

		case OP_SSE_STORE: {
			LLVMValueRef dst_vec = convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			mono_llvm_build_aligned_store (llvm::wrap (builder), rhs, dst_vec, FALSE, ins->inst_c0);
			break;
		}

		case OP_SSE_STORES: {
			LLVMValueRef first_elem = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (rhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			LLVMValueRef dst = convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			mono_llvm_build_aligned_store (llvm::wrap (builder), first_elem, dst, FALSE, 1);
			break;
		}
		case OP_SSE_MOVNTPS: {
			LLVMValueRef addr = convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef store = mono_llvm_build_aligned_store (llvm::wrap (builder), rhs, addr, FALSE, ins->inst_c0);
		...
		case OP_SSE_SHUFFLE: {
			LLVMValueRef shuffle_vec = create_const_vector_4_i32 (
				...
				((ins->inst_c0 >> 6) & 0x3) + 4);
			values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (rhs), llvm::unwrap (shuffle_vec), ""));
			break;
		}
		...
		case OP_SSE2_SHUFFLE: {
			...
			if (ins->inst_c1 == MONO_TYPE_R8) {
				...
				right_vec = llvm::wrap (llvm::UndefValue::get (llvm::FixedVectorType::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 4)));
			...
			values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (right_vec), llvm::unwrap (shuffle_vec), ""));
			break;
		}
		...
		case OP_SSE_OR: {
			...
			LLVMValueRef vec_and = llvm::wrap (builder->CreateOr (llvm::unwrap (vec_lhs_i64), llvm::unwrap (vec_rhs_i64), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (vec_and), llvm::unwrap (type_to_sse_type (ins->inst_c1)), ""));
			break;
		}
		...
		case OP_SSE_XOR: {
			LLVMValueRef vec_lhs_i64 = convert (ctx, lhs, sse_i8_t);
			LLVMValueRef vec_rhs_i64 = convert (ctx, rhs, sse_i8_t);
			LLVMValueRef vec_and = llvm::wrap (builder->CreateXor (llvm::unwrap (vec_lhs_i64), llvm::unwrap (vec_rhs_i64), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (vec_and), llvm::unwrap (type_to_sse_type (ins->inst_c1)), ""));
			break;
		}
		...
		case OP_SSE_AND: {
			LLVMValueRef vec_lhs_i64 = convert (ctx, lhs, sse_i8_t);
			LLVMValueRef vec_rhs_i64 = convert (ctx, rhs, sse_i8_t);
			LLVMValueRef vec_and = llvm::wrap (builder->CreateAnd (llvm::unwrap (vec_lhs_i64), llvm::unwrap (vec_rhs_i64), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (vec_and), llvm::unwrap (type_to_sse_type (ins->inst_c1)), ""));
			break;
		}
		...
		case OP_SSE_ANDN: {
			...
			LLVMValueRef vec_lhs_i64 = convert (ctx, lhs, sse_i8_t);
			LLVMValueRef vec_xor = llvm::wrap (builder->CreateXor (llvm::unwrap (vec_lhs_i64), llvm::unwrap (const_vector (minus_one, 2)), ""));
			LLVMValueRef vec_rhs_i64 = convert (ctx, rhs, sse_i8_t);
			LLVMValueRef vec_and = llvm::wrap (builder->CreateAnd (llvm::unwrap (vec_rhs_i64), llvm::unwrap (vec_xor), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (vec_and), llvm::unwrap (type_to_sse_type (ins->inst_c1)), ""));
			break;
		}
		...
		case OP_SSE2_DIVSD:
		case OP_SSE2_MULSD: {
			LLVMValueRef v1 = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			LLVMValueRef v2 = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (rhs), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));

			LLVMValueRef v = nullptr;
			switch (ins->opcode) {
			case OP_SSE_ADDSS:
			case OP_SSE2_ADDSD:
				v = llvm::wrap (builder->CreateFAdd (llvm::unwrap (v1), llvm::unwrap (v2), ""));
				break;
			case OP_SSE_SUBSS:
			case OP_SSE2_SUBSD:
				v = llvm::wrap (builder->CreateFSub (llvm::unwrap (v1), llvm::unwrap (v2), ""));
				break;
			case OP_SSE_DIVSS:
			case OP_SSE2_DIVSD:
				v = llvm::wrap (builder->CreateFDiv (llvm::unwrap (v1), llvm::unwrap (v2), ""));
				break;
			case OP_SSE_MULSS:
			case OP_SSE2_MULSD:
				v = llvm::wrap (builder->CreateFMul (llvm::unwrap (v1), llvm::unwrap (v2), ""));
				break;
			default:
				g_assert_not_reached ();
			}
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (lhs), llvm::unwrap (v), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			break;
		}
		...
		case OP_SSE2_CMPSD: {
			...
			LLVMValueRef cmp = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt8Ty (ctx->llvm_ctx ()), imm, false));
		...
		case OP_SSE2_CVTSI2SD64: {
			LLVMTypeRef ty = llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ()));
			switch (ins->opcode) {
			case OP_SSE2_CVTSI2SD:
			case OP_SSE2_CVTSI2SD64:
				ty = llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ()));
				break;
			}
			LLVMValueRef fp = llvm::wrap (builder->CreateSIToFP (llvm::unwrap (rhs), llvm::unwrap (ty), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (lhs), llvm::unwrap (fp), llvm::unwrap (const_int32 (0)), dname));
			break;
		}
		case OP_SSE2_PMULUDQ: {
			...
			LLVMValueRef l = llvm::wrap (builder->CreateAnd (llvm::unwrap (convert (ctx, lhs, sse_i8_t)), llvm::unwrap (mask), ""));
			LLVMValueRef r = llvm::wrap (builder->CreateAnd (llvm::unwrap (convert (ctx, rhs, sse_i8_t)), llvm::unwrap (mask), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateNUWMul (llvm::unwrap (l), llvm::unwrap (r), dname));
#endif
			break;
		}
		case OP_SSE_SQRTSS:
		case OP_SSE2_SQRTSD: {
			...
			values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (result), llvm::unwrap (lhs), llvm::unwrap (shufmask), ""));
#else
			LLVMValueRef upper = values [ins->sreg1];
			LLVMValueRef lower = values [ins->sreg2];
			LLVMValueRef scalar = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (lower), llvm::unwrap (const_int32 (0)), ""));
			LLVMValueRef result = call_intrins (ctx, simd_ins_to_intrins (ins->opcode), &scalar, dname);
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (upper), llvm::unwrap (result), llvm::unwrap (const_int32 (0)), ""));
#endif
			break;
		}
		case OP_SSE_RCPSS:
		...
		case OP_XOP_I4_X_X: {
			...
			if (ret_bool) {
				...
				values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (cmp_zero), llvm::unwrap (llvm::wrap (llvm::Type::getInt8Ty (ctx->llvm_ctx ()))), ""));
			} else {
		...
		case OP_SSE2_PSRLDQ: {
			...
			switch_ins = llvm::wrap (builder->CreateSwitch (llvm::unwrap (index), llvm::unwrap (bbs [nelems]), 0));
			for (i = 0; i < nelems; ++i) {
				LLVMAddCase (switch_ins, llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), i, false)), bbs [i]);
				builder->SetInsertPoint (llvm::unwrap (bbs [i]));
				...
				phi_values [i] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (value), llvm::unwrap (llvm::wrap (llvm::UndefValue::get (llvm::unwrap (t)))), llvm::unwrap (create_const_vector_i32 (mask_values, nelems)), ""));
				llvm::wrap (builder->CreateBr (llvm::unwrap (cbb)));
			}
			/* Default case */
			builder->SetInsertPoint (llvm::unwrap (bbs [nelems]));
			phi_values [nelems] = llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (t)));
			llvm::wrap (builder->CreateBr (llvm::unwrap (cbb)));
		...
		case OP_SSE2_PSRLQ_IMM: {
			...
			LLVMValueRef index_vect = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (t)))), llvm::unwrap (convert (ctx, index, llvm::wrap (llvm::cast<llvm::VectorType> (llvm::unwrap (t))->getElementType ()))), llvm::unwrap (const_int32 (0)), ""));
		...
		case OP_SSE2_PSHUFLW: {
			...
			/* No default case */
			switch_ins = llvm::wrap (builder->CreateSwitch (llvm::unwrap (mask), llvm::unwrap (bbs [0]), 0));
			for (int i = 0; i < ncases; ++i) {
				LLVMAddCase (switch_ins, llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), i, false)), bbs [i]);
				builder->SetInsertPoint (llvm::unwrap (bbs [i]));
				...
				phi_values [i] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (v1), llvm::unwrap (v2), llvm::unwrap (create_const_vector_i32 (mask_values, mask_len)), ""));
				llvm::wrap (builder->CreateBr (llvm::unwrap (cbb)));
			}
		...
		case OP_SSE3_MOVDDUP: {
			int mask [] = { 0, 0 };
			values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::UndefValue::get (llvm::unwrap (LLVMTypeOf (lhs))))), llvm::unwrap (create_const_vector_i32 (mask, 2)), ""));
			break;
		}
		case OP_SSE3_MOVDDUP_MEM: {
			int mask [] = { 0, 0 };
			LLVMTypeRef t = type_to_sse_type (ins->inst_c1);
			LLVMValueRef value = mono_llvm_build_load (llvm::wrap (builder), t, convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), "", FALSE);
			values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (value), llvm::unwrap (llvm::wrap (llvm::UndefValue::get (llvm::unwrap (LLVMTypeOf (value))))), llvm::unwrap (create_const_vector_i32 (mask, 2)), ""));
			break;
		}
		case OP_SSE3_MOVSHDUP: {
			int mask [] = { 1, 1, 3, 3 };
			values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (LLVMTypeOf (lhs))))), llvm::unwrap (create_const_vector_i32 (mask, 4)), ""));
			break;
		}
		...
		case OP_SSSE3_ABS: {
			...
			LLVMValueRef abs = llvm::wrap (builder->CreateSelect (llvm::unwrap (cmp), llvm::unwrap (lhs), llvm::unwrap (sub), ""));
		...
		case OP_SSSE3_ALIGNR: {
			...
			LLVMValueRef shuffled = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (convert (ctx, rhs, sse_i1_t)), llvm::unwrap (convert (ctx, lhs, sse_i1_t)), llvm::unwrap (const_vector (mask_values, 16)), ""));
			values [ins->dreg] = convert (ctx, shuffled, type_to_sse_type (ins->inst_c1));
			break;
		}
		...
		case OP_CREATE_SCALAR_UNSAFE: {
			...
			LLVMValueRef vector = (ins->opcode == OP_CREATE_SCALAR) ? llvm::wrap (llvm::Constant::getNullValue (llvm::unwrap (type))) : llvm::wrap (llvm::UndefValue::get (llvm::unwrap (type)));
			LLVMValueRef insert_pos = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false));
			LLVMValueRef val = convert_full (ctx, lhs, primitive_type_to_llvm_type (primty), primitive_type_is_unsigned (primty));
			values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (vector), llvm::unwrap (val), llvm::unwrap (insert_pos), ""));
			break;
		}
		case OP_SSE41_ROUNDP: {
			LLVMValueRef args [] = { lhs, llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), ins->inst_c0, false)) };
		...
		case OP_SSE41_DPPS_IMM: {
			LLVMValueRef args [] = { lhs, rhs, llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt8Ty (ctx->llvm_ctx ()), ins->inst_c0, false)) };
		...
		case OP_SSE41_MPSADBW_IMM: {
			LLVMValueRef args [3];
			args [0] = llvm::wrap (ctx->builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (sse_i1_t), ""));
			args [1] = llvm::wrap (ctx->builder->CreateBitCast (llvm::unwrap (rhs), llvm::unwrap (sse_i1_t), ""));
		...
		case OP_SSE41_INSERT: {
			if (ins->inst_c1 == MONO_TYPE_R4) {
				...
				args [1] = values [ins->sreg2];
				args [2] = convert (ctx, values [ins->sreg3], llvm::wrap (llvm::Type::getInt8Ty (ctx->llvm_ctx ())));
				values [ins->dreg] = call_intrins (ctx, INTRINS_SSE_INSERTPS, args, dname);
			} else {
				// other overloads are implemented with `insertelement`
				values [ins->dreg] = llvm::wrap (builder->CreateInsertElement (llvm::unwrap (values [ins->sreg1]), llvm::unwrap (convert (ctx, values [ins->sreg2], primitive_type_to_llvm_type (inst_c1_type (ins)))), llvm::unwrap (convert (ctx, values [ins->sreg3], llvm::wrap (llvm::Type::getInt8Ty (ctx->llvm_ctx ())))), dname));
			}
			break;
		}
		...
		case OP_SSE41_BLEND_IMM: {
			...
			values [ins->dreg] = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (lhs), llvm::unwrap (rhs), llvm::unwrap (mask), ""));
			break;
		}
		...
		case OP_SSE_CVTII: {
			...
			if (LLVMGetTypeKind (LLVMTypeOf (lhs)) != LLVMVectorTypeKind) {
				LLVMValueRef bitcasted = llvm::wrap (ctx->builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), ""));
				value = mono_llvm_build_aligned_load (llvm::wrap (builder), vec_type, bitcasted, "", FALSE, 1);
			} else {
				value = llvm::wrap (ctx->builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (vec_type), ""));
			}
			...
			LLVMValueRef shuffled = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (value), llvm::unwrap (llvm::wrap (llvm::UndefValue::get (llvm::unwrap (vec_type)))), llvm::unwrap (mask_vec), ""));

			if (is_signed)
				values [ins->dreg] = llvm::wrap (ctx->builder->CreateSExt (llvm::unwrap (shuffled), llvm::unwrap (dst_type), ""));
			else
				values [ins->dreg] = llvm::wrap (ctx->builder->CreateZExt (llvm::unwrap (shuffled), llvm::unwrap (dst_type), ""));
			break;
		}
		...
		case OP_SSE41_LOADANT: {
			LLVMValueRef dst_ptr = convert (ctx, lhs, llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0)));
			LLVMValueRef dst_vec = llvm::wrap (builder->CreateBitCast (llvm::unwrap (dst_ptr), llvm::unwrap (llvm::wrap (llvm::PointerType::get (ctx->llvm_ctx (), 0))), ""));
			LLVMValueRef load = mono_llvm_build_aligned_load (llvm::wrap (builder), type_to_sse_type (ins->inst_c1), dst_vec, "", FALSE, 16);
		...
		case OP_SSE41_MUL: {
			...
			for (int i = 0; i < 2; ++i) {
				LLVMValueRef padded = llvm::wrap (builder->CreateShl (llvm::unwrap (args [i]), llvm::unwrap (shift_vec), ""));
				mul_args[i] = mono_llvm_build_exact_ashr (llvm::wrap (builder), padded, shift_vec);
			}
			values [ins->dreg] = llvm::wrap (builder->CreateNSWMul (llvm::unwrap (mul_args [0]), llvm::unwrap (mul_args [1]), dname));
#endif
			break;	
		}

		case OP_SSE41_MULLO: {
			values [ins->dreg] = llvm::wrap (ctx->builder->CreateMul (llvm::unwrap (lhs), llvm::unwrap (rhs), ""));
			break;	
		}
		...
		case OP_AES_KEYGEN_IMM: {
			LLVMValueRef args [] = { lhs, llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt8Ty (ctx->llvm_ctx ()), ins->inst_c0, false)) };
		...
		case OP_XCAST: {
			...
			values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (lhs), llvm::unwrap (t), ""));
			break;
		}
		case OP_XCOMPARE_FP: {
			...
			if (ins->inst_c1 == MONO_TYPE_R8)
				values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (llvm::wrap (builder->CreateSExt (llvm::unwrap (cmp), llvm::unwrap (llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), nelems))), ""))), llvm::unwrap (LLVMTypeOf (lhs)), ""));
			else
				values [ins->dreg] = llvm::wrap (builder->CreateBitCast (llvm::unwrap (llvm::wrap (builder->CreateSExt (llvm::unwrap (cmp), llvm::unwrap (llvm::wrap (llvm::FixedVectorType::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), nelems))), ""))), llvm::unwrap (LLVMTypeOf (lhs)), ""));
			break;
		}
		case OP_XCOMPARE: {
			...
			g_assert (LLVMTypeOf (lhs) == LLVMTypeOf (rhs));
			values [ins->dreg] = llvm::wrap (builder->CreateSExt (llvm::unwrap (cmp), llvm::unwrap (LLVMTypeOf (lhs)), ""));
			break;
		}
		case OP_XEQUAL: {
			LLVMTypeRef t;
			...
			if (LLVMIsNull (lhs) || LLVMIsNull (rhs)) {
				...
				values [ins->dreg] = llvm::wrap (builder->CreateZExt (llvm::unwrap (llvm::wrap (builder->CreateICmp (to_llvm_pred (LLVMIntEQ), llvm::unwrap (values [ins->dreg]), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""))), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), dname));
				break;
			}
#endif
			LLVMTypeRef srcelemt = llvm::wrap (llvm::cast<llvm::VectorType> (llvm::unwrap (LLVMTypeOf (lhs)))->getElementType ());
			...
			LLVMTypeRef elemt;
			if (srcelemt == llvm::wrap (llvm::Type::getDoubleTy (ctx->llvm_ctx ())))
				elemt = llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()));
			else if (srcelemt == llvm::wrap (llvm::Type::getFloatTy (ctx->llvm_ctx ())))
				elemt = llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()));
			else
				elemt = srcelemt;

			t = llvm::wrap (llvm::FixedVectorType::get (llvm::unwrap (elemt), nelems));
			cmp = llvm::wrap (builder->CreateSExt (llvm::unwrap (cmp), llvm::unwrap (t), ""));
			// cmp is a <nelems x elemt> vector, each element is either 0xff... or 0
			int half = nelems / 2;
			while (half >= 1) {
				// AND the top and bottom halfes into the bottom half
				for (int i = 0; i < half; ++i)
					mask [i] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), half + i, false));
				for (int i = half; i < nelems; ++i)
					mask [i] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false));
				shuffle = llvm::wrap (builder->CreateShuffleVector (llvm::unwrap (cmp), llvm::unwrap (llvm::wrap (llvm::UndefValue::get (llvm::unwrap (t)))), llvm::unwrap (const_vector (mask, LLVMGetVectorSize (t))), ""));
				cmp = llvm::wrap (builder->CreateAnd (llvm::unwrap (cmp), llvm::unwrap (shuffle), ""));
				half = half / 2;
			}
			// Extract [0]
			LLVMValueRef first_elem = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (cmp), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), ""));
			// convert to 0/1
		...
		case OP_XEXTRACT_R4: {
			...
			case OP_XEXTRACT_I32: case OP_XEXTRACT_R4:
				mask = llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (rhst), 0x3, false)); break;
			case OP_XEXTRACT_I64: case OP_XEXTRACT_R8:
				mask = llvm::wrap (llvm::ConstantInt::get (llvm::unwrap (rhst), 0x1, false)); break;
			default:
				g_assert_not_reached ();
			}
			LLVMValueRef selector = llvm::wrap (builder->CreateAnd (llvm::unwrap (rhs), llvm::unwrap (mask), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateExtractElement (llvm::unwrap (lhs), llvm::unwrap (selector), ""));
			break;
		}
		case OP_POPCNT32:
		...
		case OP_BEXTR64: {
			LLVMValueRef args [2];
			args [0] = lhs;
			args [1] = convert (ctx, rhs, ins->opcode == OP_BEXTR32 ? llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ())) : llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))); // cast ushort to u32/u64
		...
		case OP_MULX_HL64: {
			...
			LLVMValueRef lx = llvm::wrap (ctx->builder->CreateZExt (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt128Ty (ctx->llvm_ctx ()))), ""));
			LLVMValueRef rx = llvm::wrap (ctx->builder->CreateZExt (llvm::unwrap (rhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt128Ty (ctx->llvm_ctx ()))), ""));
			LLVMValueRef mulx = llvm::wrap (ctx->builder->CreateMul (llvm::unwrap (lx), llvm::unwrap (rx), ""));
			if (!only_high) {
				LLVMValueRef lowx = llvm::wrap (ctx->builder->CreateTrunc (llvm::unwrap (mulx), llvm::unwrap (is_64 ? llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())) : llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), ""));
				llvm::wrap (ctx->builder->CreateStore (llvm::unwrap (lowx), llvm::unwrap (values [ins->sreg3])));
			}
			LLVMValueRef shift = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt128Ty (ctx->llvm_ctx ()), is_64 ? 64 : 32, false));
			LLVMValueRef highx = llvm::wrap (ctx->builder->CreateLShr (llvm::unwrap (mulx), llvm::unwrap (shift), ""));
			values [ins->dreg] = llvm::wrap (ctx->builder->CreateTrunc (llvm::unwrap (highx), llvm::unwrap (is_64 ? llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ())) : llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), ""));
			break;
		}
		case OP_PEXT32:
		case OP_PEXT64: {
...
// Shared between ARM64 and X86
		...
		case OP_LZCNT64: {
			...
			args [0] = lhs;
			args [1] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt1Ty (ctx->llvm_ctx ()), 1, false));
			values [ins->dreg] = llvm::wrap (builder->CreateCall (llvm::cast<llvm::FunctionType> (llvm::unwrap (LLVMGlobalGetValueType (get_intrins (ctx, ins->opcode == OP_LZCNT32 ? INTRINS_CTLZ_I32 : INTRINS_CTLZ_I64)))), llvm::unwrap (get_intrins (ctx, ins->opcode == OP_LZCNT32 ? INTRINS_CTLZ_I32 : INTRINS_CTLZ_I64)), gep_index_list (args, 2), ""));
			break;
		}
#endif
		...
		case OP_XOP_I4_I4_I8: {
			...
			if (zext_last)
				arg1 = llvm::wrap (ctx->builder->CreateZExt (llvm::unwrap (arg1), llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), ""));
		...
		case OP_LSCNT64: {
			...
			LLVMValueRef shr = llvm::wrap (builder->CreateAShr (llvm::unwrap (lhs), llvm::unwrap (ins->opcode == OP_LSCNT32 ? 
				llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 31, false)) : 
				llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), 63, false))), ""));
			LLVMValueRef one = ins->opcode == OP_LSCNT32 ? 
				llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 1, false)) : 
				llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt64Ty (ctx->llvm_ctx ()), 1, false));
			LLVMValueRef xor = llvm::wrap (builder->CreateXor (llvm::unwrap (shr), llvm::unwrap (lhs), ""));
			LLVMValueRef mul = llvm::wrap (builder->CreateShl (llvm::unwrap (xor), llvm::unwrap (one), ""));
			LLVMValueRef add = llvm::wrap (builder->CreateOr (llvm::unwrap (mul), llvm::unwrap (one), ""));
			
			LLVMValueRef args [2];
			args [0] = add;
			args [1] = llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt1Ty (ctx->llvm_ctx ()), 0, false));
			values [ins->dreg] = llvm::wrap (builder->CreateCall (llvm::cast<llvm::FunctionType> (llvm::unwrap (LLVMGlobalGetValueType (get_intrins (ctx, ins->opcode == OP_LSCNT32 ? INTRINS_CTLZ_I32 : INTRINS_CTLZ_I64)))), llvm::unwrap (get_intrins (ctx, ins->opcode == OP_LSCNT32 ? INTRINS_CTLZ_I32 : INTRINS_CTLZ_I64)), gep_index_list (args, 2), ""));
			break;
		}
		case OP_ARM64_SMULH:
		case OP_ARM64_UMULH: {
			...
			if (ins->opcode == OP_ARM64_SMULH) {
				op1 = llvm::wrap (builder->CreateSExt (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt128Ty (ctx->llvm_ctx ()))), ""));
				op2 = llvm::wrap (builder->CreateSExt (llvm::unwrap (rhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt128Ty (ctx->llvm_ctx ()))), ""));
			} else {
				op1 = llvm::wrap (builder->CreateZExt (llvm::unwrap (lhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt128Ty (ctx->llvm_ctx ()))), ""));
				op2 = llvm::wrap (builder->CreateZExt (llvm::unwrap (rhs), llvm::unwrap (llvm::wrap (llvm::Type::getInt128Ty (ctx->llvm_ctx ()))), ""));
			}
			LLVMValueRef mul = llvm::wrap (builder->CreateMul (llvm::unwrap (op1), llvm::unwrap (op2), ""));
			LLVMValueRef hi64 = llvm::wrap (builder->CreateLShr (llvm::unwrap (mul), llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt128Ty (ctx->llvm_ctx ()), 64, false))), ""));
			values [ins->dreg] = llvm::wrap (builder->CreateTrunc (llvm::unwrap (hi64), llvm::unwrap (llvm::wrap (llvm::Type::getInt64Ty (ctx->llvm_ctx ()))), ""));
			break;
		}
#endif
		...
		case OP_CALL_HANDLER: {
			...
			g_assert (lhs);
			llvm::wrap (builder->CreateStore (llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), g_slist_length (bb_list) + 1, false))), llvm::unwrap (lhs)));
		...
		case OP_ENDFINALLY: {
			...
			if (!is_fault) {
				...
				val = llvm::wrap (builder->CreateLoad (llvm::unwrap (llvm::wrap (llvm::Type::getInt32Ty (ctx->llvm_ctx ()))), llvm::unwrap (lhs), ""));

				/* Reset the variable */
				llvm::wrap (builder->CreateStore (llvm::unwrap (llvm::wrap (llvm::ConstantInt::get (llvm::Type::getInt32Ty (ctx->llvm_ctx ()), 0, false))), llvm::unwrap (lhs)));

				/* Branch to either resume_bb, or to the bblocks in bb_list */
				switch_ins = llvm::wrap (builder->CreateSwitch (llvm::unwrap (val), llvm::unwrap (resume_bb), g_slist_length (bb_list)));
				/*
	...
	if (!has_terminator && bb->next_bb && (bb == cfg->bb_entry || bb->in_count > 0)) {
		llvm::wrap (builder->CreateBr (llvm::unwrap (get_bb (ctx, bb->next_bb))));
	}
