...
/// says the inline pays (runtime/trivial-inlines.cpp). The translator declares a
...
/// engine's gates run here: a test says which body it wants inlined.
llvm::Function *inline_method_into (Translation &into, const std::string &image,
                                    const std::string &method);
