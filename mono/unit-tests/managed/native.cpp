...
//
// The marshalling wrapper the runtime builds is sorted by the call's
// signature.  These cover the shapes it sorts into: each arity up to six over
// void and non-void, a struct returned in a register pair, a buffer written
// through, a callback, an errno, and a variadic call.  A test picks the shape
// it is about, so each body only has to be small and predictable.
//
