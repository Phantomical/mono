...
TransformData::interp_optimize_bblocks ()
{
	...
	while (TRUE) {
		...
		if (next_bb->in_count == 0 && !next_bb->eh_block) {
			...
			continue;
		} else if (bb->out_count == 1 && bb->out_bb[0] == next_bb && next_bb->in_count == 1
		           && !next_bb->eh_block) {
...
TransformData::interp_local_deadce (int *local_ref_count)
{
	...
	for (size_t i = 0; i < locals.size (); i++) {
		...
		if (!local_ref_count[i] && !locals[i].indirects
	...
	for (InterpBasicBlock *bb : blocks_from (entry_bb)) {
		for (InterpInst *ins : *bb) {
			if (MINT_IS_MOV (ins->opcode) || MINT_IS_LDC_I4 (ins->opcode)
			    || ins->opcode == MINT_LDC_I8 || ins->opcode == MINT_MONO_LDPTR
...
#define INTERP_FOLD_CONV(opcode, val_type_dst, field_dst, val_type_src, field_src, cast_type) \
	case opcode:                                                                              \
		if (val->type != val_type_src)                                                        \
			return ins;                                                                       \
		result.type = val_type_dst;                                                           \
		result.field_dst = (cast_type) val->field_src;                                        \
		break;

#define INTERP_FOLD_CONV_FULL(opcode, val_type_dst, field_dst, val_type_src, field_src, cast_type, \
                              cond)                                                                \
	case opcode:                                                                                   \
		if (val->type != val_type_src)                                                             \
			return ins;                                                                            \
...
TransformData::interp_fold_unop (LocalValue *local_defs, int *local_ref_count, InterpInst *ins)
{
	...
	switch (ins->opcode) {
		...
		INTERP_FOLD_CONV_FULL (MINT_CONV_INDEX_I8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, l, gint32,
		                       (guint64) val->l <= G_MAXINT32);
		...
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I1_I4, LOCAL_VALUE_I4, i, LOCAL_VALUE_I4, i, gint8,
		                       val->i >= G_MININT8 && val->i <= G_MAXINT8);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I1_I8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, l, gint8,
		                       val->l >= G_MININT8 && val->l <= G_MAXINT8);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I1_U4, LOCAL_VALUE_I4, i, LOCAL_VALUE_I4, i, gint8,
		                       val->i >= 0 && val->i <= G_MAXINT8);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I1_U8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, l, gint8,
		                       val->l >= 0 && val->l <= G_MAXINT8);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_U1_I4, LOCAL_VALUE_I4, i, LOCAL_VALUE_I4, i, guint8,
		                       val->i >= 0 && val->i <= G_MAXUINT8);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_U1_I8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, l, guint8,
		                       val->l >= 0 && val->l <= G_MAXUINT8);

		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I2_I4, LOCAL_VALUE_I4, i, LOCAL_VALUE_I4, i, gint16,
		                       val->i >= G_MININT16 && val->i <= G_MAXINT16);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I2_I8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, i, gint16,
		                       val->l >= G_MININT16 && val->l <= G_MAXINT16);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I2_U4, LOCAL_VALUE_I4, i, LOCAL_VALUE_I4, i, gint16,
		                       val->i >= 0 && val->i <= G_MAXINT16);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I2_U8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, l, gint16,
		                       val->l >= 0 && val->l <= G_MAXINT16);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_U2_I4, LOCAL_VALUE_I4, i, LOCAL_VALUE_I4, i, guint16,
		                       val->i >= 0 && val->i <= G_MAXUINT16);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_U2_I8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, l, guint16,
		                       val->l >= 0 && val->l <= G_MAXUINT16);

		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I4_U4, LOCAL_VALUE_I4, i, LOCAL_VALUE_I4, i, gint32,
		                       val->i >= 0);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I4_I8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, l, gint32,
		                       val->l >= G_MININT32 && val->l <= G_MAXINT32);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I4_U8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, l, gint32,
		                       val->l >= 0 && val->l <= G_MAXINT32);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_U4_I4, LOCAL_VALUE_I4, i, LOCAL_VALUE_I4, i, guint32,
		                       val->i >= 0);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_U4_I8, LOCAL_VALUE_I4, i, LOCAL_VALUE_I8, l, guint32,
		                       val->l >= 0 && val->l <= G_MAXINT32);

		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_I8_U8, LOCAL_VALUE_I8, l, LOCAL_VALUE_I8, l, gint64,
		                       val->l >= 0);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_U8_I4, LOCAL_VALUE_I8, l, LOCAL_VALUE_I4, i, guint64,
		                       val->i >= 0);
		INTERP_FOLD_CONV_FULL (MINT_CONV_OVF_U8_I8, LOCAL_VALUE_I8, l, LOCAL_VALUE_I8, l, guint64,
		                       val->l >= 0);
...
#define INTERP_FOLD_BINOP(opcode, local_type, field, op)          \
	case opcode:                                                  \
		if (val1->type != local_type || val2->type != local_type) \
			return ins;                                           \
...
TransformData::interp_fold_binop (LocalValue *local_defs, int *local_ref_count, InterpInst *ins)
{
	...
	switch (ins->opcode) {
		...
		INTERP_FOLD_BINOP_FULL (MINT_DIV_I4, LOCAL_VALUE_I4, i, /, gint32,
		                        val2->i != 0 && (val1->i != G_MININT32 || val2->i != -1));
		INTERP_FOLD_BINOP_FULL (MINT_DIV_I8, LOCAL_VALUE_I8, l, /, gint64,
		                        val2->l != 0 && (val1->l != G_MININT64 || val2->l != -1));
		...
		INTERP_FOLD_BINOP_FULL (MINT_REM_I4, LOCAL_VALUE_I4, i, %, gint32,
		                        val2->i != 0 && (val1->i != G_MININT32 || val2->i != -1));
		INTERP_FOLD_BINOP_FULL (MINT_REM_I8, LOCAL_VALUE_I8, l, %, gint64,
		                        val2->l != 0 && (val1->l != G_MININT64 || val2->l != -1));
                                          ...
                                          int *local_ref_count, InterpInst *ins)
{
	...
	switch (ins->opcode) {
		...
		INTERP_FOLD_BINOP_BR (MINT_BGE_UN_I4_S, LOCAL_VALUE_I4,
		                      (guint32) val1->i >= (guint32) val2->i);
		INTERP_FOLD_BINOP_BR (MINT_BGE_UN_I8_S, LOCAL_VALUE_I8,
		                      (guint64) val1->l >= (guint64) val2->l);
		INTERP_FOLD_BINOP_BR (MINT_BGT_UN_I4_S, LOCAL_VALUE_I4,
		                      (guint32) val1->i > (guint32) val2->i);
		INTERP_FOLD_BINOP_BR (MINT_BGT_UN_I8_S, LOCAL_VALUE_I8,
		                      (guint64) val1->l > (guint64) val2->l);
		INTERP_FOLD_BINOP_BR (MINT_BLE_UN_I4_S, LOCAL_VALUE_I4,
		                      (guint32) val1->i <= (guint32) val2->i);
		INTERP_FOLD_BINOP_BR (MINT_BLE_UN_I8_S, LOCAL_VALUE_I8,
		                      (guint64) val1->l <= (guint64) val2->l);
		INTERP_FOLD_BINOP_BR (MINT_BLT_UN_I4_S, LOCAL_VALUE_I4,
		                      (guint32) val1->i < (guint32) val2->i);
		INTERP_FOLD_BINOP_BR (MINT_BLT_UN_I8_S, LOCAL_VALUE_I8,
		                      (guint64) val1->l < (guint64) val2->l);
		...
		for (ins = bb->first_ins; ins != NULL; ins = ins->next) {
			...
			for (int i = 0; i < sregs_count; i++) {
				...
				if (local_defs[sregs[i]].type == LOCAL_VALUE_LOCAL) {
					...
					if (local_defs[cprop_local].ins != NULL
					    && local_defs[cprop_local].def_index > local_defs[sregs[i]].def_index)
						continue;
			...
			if (opcode == MINT_MOV_4 || opcode == MINT_MOV_8 || opcode == MINT_MOV_VT) {
				...
				} else if (local_defs[sreg].type == LOCAL_VALUE_I4
				           || local_defs[sreg].type == LOCAL_VALUE_I8) {
			...
			} else if ((ins->opcode == MINT_NEWOBJ_FAST || ins->opcode == MINT_NEWOBJ_VT_FAST)
