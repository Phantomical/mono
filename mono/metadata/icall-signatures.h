...
//
// G_EXTERN_C_VAR because mono/llvm/ registers icalls from C++ and these are
// defined in mono/metadata/icall.c.  The Itanium ABI leaves a global
// variable's name alone whichever language declared it, so the mismatch
// resolves by accident there; MSVC decorates the C++ spelling and the
// reference finds nothing.
#define ICALL_SIG(n, types) G_EXTERN_C_VAR MonoMethodSignature * const ICALL_SIG_NAME (n, types);
