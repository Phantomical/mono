...
// A copy of llvm/lib/Analysis/InlineCost.cpp at llvmorg-23.1.1. CallAnalyzer is
// in no header, so a subclass outside LLVM cannot reach it, and a copy is what
// lets the model read what the managed metadata knows about a call site.
//
...
#define DEBUG_TYPE "mono-inline-cost"
...
static cl::opt<int>
    DefaultThreshold("mono-inlinedefault-threshold", cl::Hidden, cl::init(225),
...
static cl::opt<bool> IgnoreTTIInlineCompatible(
    "mono-ignore-tti-inline-compatible", cl::Hidden, cl::init(false),
...
static cl::opt<bool> PrintInstructionComments(
    "mono-print-instruction-comments", cl::Hidden, cl::init(false),
    cl::desc("Prints comments for instruction based on inline cost analysis"));

static cl::opt<int> InlineThreshold(
    "mono-inline-threshold", cl::Hidden, cl::init(225),
    cl::desc("Control the amount of inlining to perform (default = 225)"));

static cl::opt<int> HintThreshold(
    "mono-inlinehint-threshold", cl::Hidden, cl::init(325),
    cl::desc("Threshold for inlining functions with inline hint"));

static cl::opt<int>
    ColdCallSiteThreshold("mono-inline-cold-callsite-threshold", cl::Hidden,
                          cl::init(45),
                          cl::desc("Threshold for inlining cold callsites"));

static cl::opt<bool> InlineEnableCostBenefitAnalysis(
    "mono-inline-enable-cost-benefit-analysis", cl::Hidden, cl::init(false),
...
static cl::opt<int> InlineSavingsMultiplier(
    "mono-inline-savings-multiplier", cl::Hidden, cl::init(8),
...
static cl::opt<int> InlineSavingsProfitableMultiplier(
    "mono-inline-savings-profitable-multiplier", cl::Hidden, cl::init(4),
    cl::desc("A multiplier on top of cycle savings to decide whether the "
             "savings won't justify the cost"));

static cl::opt<int>
    InlineSizeAllowance("mono-inline-size-allowance", cl::Hidden, cl::init(100),
...
static cl::opt<int> ColdThreshold(
    "mono-inlinecold-threshold", cl::Hidden, cl::init(45),
    cl::desc("Threshold for inlining functions with cold attribute"));

static cl::opt<int>
    HotCallSiteThreshold("mono-hot-callsite-threshold", cl::Hidden, cl::init(3000),
                         cl::desc("Threshold for hot callsites "));

static cl::opt<int> LocallyHotCallSiteThreshold(
    "mono-locally-hot-callsite-threshold", cl::Hidden, cl::init(525),
    cl::desc("Threshold for locally hot callsites "));

static cl::opt<int> ColdCallSiteRelFreq(
    "mono-cold-callsite-rel-freq", cl::Hidden, cl::init(2),
...
static cl::opt<size_t> RecurStackSizeThreshold(
    "mono-recursive-inline-max-stacksize", cl::Hidden,
...
static cl::opt<bool> OptComputeFullInlineCost(
    "mono-inline-cost-full", cl::Hidden,
    cl::desc("Compute the full inline cost of a call site even when the cost "
             "exceeds the threshold."));

static cl::opt<bool> InlineCallerSupersetNoBuiltin(
    "mono-inline-caller-superset-nobuiltin", cl::Hidden, cl::init(true),
    cl::desc("Allow inlining when caller has a superset of callee's nobuiltin "
             "attributes."));

static cl::opt<bool> DisableGEPConstOperand(
    "mono-disable-gep-const-evaluation", cl::Hidden, cl::init(false),
    cl::desc("Disables evaluation of GetElementPtr with constant operands"));

static cl::opt<bool> InlineAllViableCalls(
    "mono-inline-all-viable-calls", cl::Hidden, cl::init(false),
    cl::desc("Inline all viable calls, even if they exceed the inlining "
             "threshold"));
// InlineCost.h declares the CallBase overload of getStringFnAttrAsInt (), so
// the calls below reach the dylib's copy of it. A copy of our own makes each of
// those calls ambiguous: the argument is an llvm type, so argument-dependent
// lookup finds the exported overload beside ours. The other two overloads are
// in no header and have to stand here.
//
// InlineConstants::getInstrCost () goes with them. It reports InstrCost to a
// caller outside this file, and this copy has no such caller.
...
class InlineCostCallAnalyzer final : public CallAnalyzer {
  ...
  std::optional<bool> costBenefitAnalysis() {
    ...
    CycleSavings += mono::getCallsiteCost(TTI, this->CandidateCall, DL);
  ...
  InlineResult onAnalysisStart() override {
    ...
    addCost(-mono::getCallsiteCost(TTI, this->CandidateCall, DL));
...
private:
  ...
  InlineResult onAnalysisStart() override {
    increment(InlineCostFeatureIndex::callsite_cost,
              -1 * mono::getCallsiteCost(TTI, this->CandidateCall, DL));
...
std::optional<InlineResult> getAttributeBasedInliningDecision(
  ...
  if (Call.hasFnAttr(Attribute::AlwaysInline)) {
    ...
    auto IsViable = mono::isInlineViable(*Callee);
...
InlineCost getInlineCost(
  ...
  if (InlineAllViableCalls && mono::isInlineViable(*Callee).isSuccess())
    return llvm::InlineCost::getAlways(
        "Inlining forced by -mono-inline-all-viable-calls");
...
InlineResult isInlineViable(Function &F) {
...
InlineParams getInlineParams() {
  return mono::getInlineParams(DefaultThreshold);
}

InlineParams getInlineParamsFromOptLevel(unsigned OptLevel) {
  auto Params = mono::getInlineParams(
      OptLevel > 2 ? InlineConstants::OptAggressiveThreshold : DefaultThreshold);
...
// The copy leaves out InlineCostAnnotationPrinterPass::run (). It defines a
// member of a class InlineCost.h declares, so it collides with the definition
// the dylib carries. -mono-print-instruction-comments turns the same annotation
// on here.
