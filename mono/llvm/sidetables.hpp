 ...
 *
 * `.mono_lines` is the IL-offset map: which IL instruction was in effect at each
 * code offset, which is what a managed stack trace prints and what the soft
 * debugger's sequence points are recovered from. The offsets are instruction
 * starts, so a frame below the top names the call or the throw that gave up
 * control. A symbol reader turns that back into a line, and it must take the
 * statement at or before the offset to get one. Same block-per-function shape as
 * `.mono_unwind` and for the same reason.
 *
 *   Header (20 bytes, little-endian):
