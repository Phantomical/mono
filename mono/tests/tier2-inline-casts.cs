 ...
 * off in both (--llvm-opt=-mono-inline-il-limit=0), so an inline this reads is
 * the cost model's.
 *
...
static class Program {
	...
	public static int Main ()
	{
		...
		if (answers)
			Check (inlined_weigh, "an answered cascade inlines the body behind it");
		else
			Check (!inlined_weigh,
				"an answered cascade is what inlines the body behind it");
